"""CAN Interactive Plot — main entry point.

Combines the interactive plot pipeline (InteractivePlot layers a-e) with the
CAN KPI pipeline (CAN layers a-d). During parsing, per-sensor CAN KPI requests
are pushed to the CAN KPI ZMQ server (can_kpi_server.py), whose generated KPI
HTML is linked into the interactive reports as KPI tabs.

Usage
-----
    python can_inplot_main.py <config.xml> <inputs.json> [output_directory] [plot_config.json]

Also supported:
    python can_inplot_main.py <config.xml> <inputs.json> <plot_config.json> [output_directory]
"""

import sys
import os

# Allow running as a script: python can_inplot/can_inplot_main.py ...
...
from can_inplot.a_config_layer.xml_config_parser import XmlConfigParser
from can_inplot.a_config_layer.json_parser_factory import JSONParserFactory
from can_inplot.c_data_storage.config_loader import (
    load_plot_config,
    get_plot_config_source,
)
from can_inplot.b_persistence_layer.hdf_processor_factory import HdfProcessorFactory
from can_inplot.d_business_layer.utils import time_taken, LoggerSetup
from can_inplot.d_business_layer.path_utils import set_default_umask
import asyncio
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3 or len(sys.argv) > 5:
        print(
            "Usage: python can_inplot_main.py <config.xml> <inputs.json> [output_directory] [plot_config.json]"
        )
        print(
            "Also supported: python can_inplot_main.py <config.xml> <inputs.json> <plot_config.json> [output_directory]"
        )
        sys.exit(1)
