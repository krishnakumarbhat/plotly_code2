import os  # noqa: F401
from can_inplot.e_presentation_layer.front_end import html_template
import logging
import json
...
import re
import math
from can_inplot.e_presentation_layer.main_front_end import css, js, master_index_css, master_index_js
...
class HtmlGenerator:
    ...
    def __init__(
        ...
        self._write_overview_line_data()
        self.generate_and_save_html_files()

    OVERVIEW_MAX_POINTS = 2000

    def _write_overview_line_data(self):
        """Persist a compact input/output line series for the Timeline Overview page.

        Picks the first scatter-style figure of this stream, downsamples it,
        and writes overview_line.json next to the stream's HTML pages so the
        overview index can embed real line plots without re-parsing HDF.
        """
        try:
            ordered = sorted(
                (p for plots in (self.plots or {}).values() for p in plots),
                key=lambda p: 0 if ";scatter" in p["key"].lower() else 1,
            )
            entry = None
            for plot_info in ordered:
                try:
                    with open(plot_info["path"], "r", encoding="utf-8") as fh:
                        fig_json = json.load(fh)
                    traces = fig_json.get("data") or []
                    if not traces:
                        continue
                    x = list(traces[0].get("x") or [])
                    y_in = list(traces[0].get("y") or [])
                    y_out = list(traces[1].get("y") or []) if len(traces) > 1 else []
                    n = len(x)
                    if n < 2 or len(y_in) != n:
                        continue
                    step = max(1, n // self.OVERVIEW_MAX_POINTS)
                    layout_title = {}
                    try:
                        layout_title = (((fig_json.get("layout") or {}).get("xaxis") or {}).get("title") or {})
                    except Exception:
                        pass
                    entry = {
                        "sensor": self.sensor_position,
                        "stream": self.stream_name,
                        "signal": str(plot_info["key"]).split(";")[0],
                        "x_label": layout_title.get("text") or "ScanIndex",
                        "x": x[::step],
                        "y_input": y_in[::step],
                        "y_output": [y_out[i] if i < len(y_out) else None for i in range(0, n, step)],
                    }
                    break
                except Exception:
                    continue
            if entry:
                out_path = Path(self.folder_structure["streams"]) / "overview_line.json"
                out_path.parent.mkdir(parents=True, exist_ok=True)
                with open(out_path, "w", encoding="utf-8") as fh:
                    json.dump(entry, fh)
        except Exception:
            self.logger.debug("Timeline overview line capture failed", exc_info=True)

    @classmethod
    def _collect_timeline_plots(cls, root: Path) -> str:
        """Render embedded line charts from every overview_line.json under root."""
        try:
            import numpy as np
        ...
        sections = []
        first = True
        entries = sorted(root.rglob("overview_line.json"))
        for entry_path in entries[:64]:  # cap: overview stays fast on huge runs
            try:
                with open(entry_path, "r", encoding="utf-8") as fh:
                    e = json.load(fh)
                rel = entry_path.parent.relative_to(root).as_posix()
                fig = go.Figure()
                fig.add_trace(go.Scattergl(
                    x=e.get("x") or [], y=e.get("y_input") or [],
                    mode="lines", name="INPUT", line=dict(color="#d62728"),
                ))
                y_out = e.get("y_output")
                if y_out:
                    fig.add_trace(go.Scattergl(
                        x=e.get("x") or [], y=y_out,
                        mode="lines", name="OUTPUT", line=dict(color="#1f77b4"),
                    ))
                # Quick agreement stat over overlapping finite points
                agree_txt = ""
                try:
                    yi = np.asarray(e.get("y_input"), dtype=float)
                    yo = np.asarray([v if v is not None else np.nan for v in (y_out or [])], dtype=float)
                    mask = np.isfinite(yi) & np.isfinite(yo)
                    if mask.any():
                        denom = np.maximum(np.abs(yi[mask]), 1e-9)
                        agree = float(np.mean(np.abs(yi[mask] - yo[mask]) / denom <= 0.02) * 100.0)
                        agree_txt = f'<span class="ov-agree">agreement(±2%): {agree:.1f}%</span>'
                except Exception:
                    pass
                fig.update_layout(
                    title=f"{e.get('signal')} — {rel}",
                    xaxis_title=e.get("x_label") or "ScanIndex",
                    yaxis_title=e.get("signal"),
                    hovermode="closest", margin=dict(l=40, r=20, t=50, b=40),
                )
                sections.append(
                    f'<section class="ov-plot"><h3>{e.get("signal")} <small>({rel})</small> {agree_txt}</h3>'
        ...
        if not sections:
            return ""
        style = ("<style>.timeline-plots{padding:16px}.timeline-plots h2{margin:6px 0 12px}"
                 ".ov-plot{background:#fff;border:1px solid #d9e3ec;border-radius:14px;"
                 "padding:10px 14px;margin-bottom:14px}.ov-plot h3 small{color:#617789;font-weight:400}"
                 ".ov-agree{font-size:.85rem;color:#0f4760;background:#eef7fb;border-radius:8px;padding:2px 8px;margin-left:8px}</style>")
        return style + '<div class="timeline-plots"><h2>Signal Timelines</h2>' + "".join(sections) + "</div>"
    ...
    def generate_and_save_html_files(self):
        ...
        if os.environ.get('INTERACTIVE_PLOT_ENABLE_KPI', '0') == '1':
            try:
                from can_inplot.kpi_client.can_kpi_integration import CanKpiIntegration

                kpi_path = CanKpiIntegration.receive_html_path_from_can_kpi_server()
                if not kpi_path:
    ...
    def _request_kpi_html_directly(self) -> Optional[str]:
        """Request HTML path directly from the CAN KPI server via ZMQ."""
        try:
            import zmq
            ...
            host = os.environ.get("CAN_KPI_SERVER_HOST", "127.0.0.1")
            port = int(os.environ.get("CAN_KPI_SERVER_PORT", "6000"))
    ...
    @classmethod
    def create_timeline_overview(cls, output_dir: str, base_filename: str = "timeline_overview"):
        """
        Create the Timeline Overview HTML: embedded per-stream signal line
        plots plus the nested sensor/stream navigation (formerly "master index").

        Parameters:
            output_dir: Root output directory
            base_filename: Base name for the overview file
        """
        overview_path = Path(output_dir) / f"{base_filename}.html"

        # Find all HTML files in the output directory
        html_files = []
        for html_file in Path(output_dir).rglob("*.html"):
            if html_file.name == f"{base_filename}.html" or html_file.name == "master_index.html":
                continue
            # Skip navigation/report scaffolding so it doesn't appear as a
            # bogus "Others" sensor: per-base indexes and CAN KPI index pages
            # are linked from the metadata cards instead.
            if html_file.name == "can_kpi_index.html":
                continue
            if html_file.name == Path(html_file.parent).name + ".html":
                continue
        ...
        # Generate overview HTML (navigation + embedded timeline line plots)
        master_html = cls._generate_master_index_html(sensor_stream_data, base_filename, anchor_dir=Path(output_dir))
        plots_html = cls._collect_timeline_plots(Path(output_dir))
        if plots_html and "<body" in master_html:
            import re as _re
            master_html = _re.sub(r"(<body[^>]*>)", r"\1" + plots_html.replace("\\", "\\\\"), master_html, count=1)

        # Save overview
        with open(overview_path, "w", encoding="utf-8") as f:
            f.write(master_html)

        logging.getLogger(cls.__name__).info(f"Timeline overview created: {overview_path}")
        return overview_path

    # Backward-compatible alias (output file is now timeline_overview.html)
    @classmethod
    def create_master_index(cls, output_dir: str, base_filename: str = "timeline_overview"):
        return cls.create_timeline_overview(output_dir, base_filename)
    ...
    def create_base_index(cls, output_dir: str, base_filename: str):
        ...
        # Organize and render
        sensor_stream_data = cls._organize_by_sensor_stream(html_files)
        master_html = cls._generate_master_index_html(sensor_stream_data, base_filename, anchor_dir=base_folder)
    ...
    @classmethod
    def _find_kpi_stats_json(cls, file_path: Path) -> Optional[Path]:
        """Locate the CAN KPI stats sidecar next to a KPI HTML page
        (e.g. ``<name>_kpi.html`` -> ``<name>_kpi_stats.json``)."""
        try:
            folder = file_path.parent
            stem = file_path.stem
            candidates = [
                folder / f"{stem}_stats.json",
                folder / f"{stem.replace('_kpi', '_kpi_stats')}.json",
            ]
            ...
            for categories in streams.values():
                for category_name, plots in categories.items():
                    ...
                    for plot_info in plots:
                        ...
                        # CAN KPI stats sidecar carries aligned scan counts directly.
                        stats_path = cls._find_kpi_stats_json(file_path)
                        if stats_path:
                            try:
                                data = json.loads(stats_path.read_text(encoding="utf-8"))
                                aligned = float(data.get("aligned_scans") or float("nan"))
                                if not math.isnan(aligned):
                                    has_scan_data = True
                                    common_total += aligned
                                    input_only_total += 0.0
                                    output_only_total += 0.0
                                    continue
                            except Exception:
    ...
    def _find_can_kpi_index(cls, anchor_dir: Path) -> Optional[str]:
        ...
        try:
            if not anchor_dir or not anchor_dir.exists():
                return None
            matches = sorted(anchor_dir.rglob("can_kpi_index.html"))
            if not matches:
                return None
            rel = matches[0].relative_to(anchor_dir)
            return rel.as_posix()
        except Exception:
            return None

    @classmethod
    def _generate_master_index_html(cls, sensor_stream_data: dict, base_filename: str, anchor_dir: Optional[Path] = None) -> str:
        ...
        kpi_index_rel = cls._find_can_kpi_index(anchor_dir)
        kpi_link_html = ""
        if kpi_index_rel:
            kpi_link_html = f'''
            <div class="metadata-item">
                <strong>CAN KPI Reports</strong>
                <div><a href="{kpi_index_rel}">Open CAN KPI Index →</a></div>
            </div>
            '''
        ...
        <head>
            <meta charset="UTF-8">
            <meta name="viewport" content="width=device-width, initial-scale=1.0">
            <title>Timeline Overview — Interactive Plot Report</title>
            <style>
                {master_index_css}
            </style>
        </head>
        <body>
            <div class="container">
                <div class="header">
                    <h1>📊 Timeline Overview — Interactive Plot Report</h1>
