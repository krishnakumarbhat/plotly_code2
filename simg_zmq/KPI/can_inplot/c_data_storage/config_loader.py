import json
import logging
import os
import re
from typing import Dict, Optional, Tuple

from can_inplot.c_data_storage.config_storage import Gen7V1_V2


_ACTIVE_PLOT_CONFIG = Gen7V1_V2
_ACTIVE_PLOT_CONFIG_SOURCE = "can_inplot/c_data_storage/config_storage.py"



def _normalize_config(raw_config: dict) -> dict:
    if not isinstance(raw_config, dict):
        raise ValueError("Config content must be a JSON object")

    if "Gen7V1_V2" in raw_config and isinstance(raw_config["Gen7V1_V2"], dict):
        return raw_config["Gen7V1_V2"]

    return raw_config



def _is_valid_plot_config(candidate: dict) -> bool:
    if not isinstance(candidate, dict) or not candidate:
        return False

    has_stream_dict = any(isinstance(value, dict) for value in candidate.values())
    return has_stream_dict



def load_plot_config(config_json_path: Optional[str] = None) -> Tuple[Dict, str]:
    global _ACTIVE_PLOT_CONFIG, _ACTIVE_PLOT_CONFIG_SOURCE

    resolved_path = config_json_path
    if not resolved_path:
        default_path = os.path.join(os.getcwd(), "config.json")
        if os.path.exists(default_path):
            resolved_path = default_path

    if resolved_path:
        try:
            if os.path.getsize(resolved_path) == 0:
                raise ValueError("Config JSON file is empty")

            with open(resolved_path, "r", encoding="utf-8") as cfg_file:
                loaded = json.load(cfg_file)

            normalized = _normalize_config(loaded)
            if not _is_valid_plot_config(normalized):
                raise ValueError("Config JSON is not in expected stream/signal format")

            _ACTIVE_PLOT_CONFIG = normalized
            _ACTIVE_PLOT_CONFIG_SOURCE = resolved_path
            logging.info("Using plot configuration from JSON: %s", resolved_path)
            return _ACTIVE_PLOT_CONFIG, _ACTIVE_PLOT_CONFIG_SOURCE
        except Exception as ex:
            logging.warning(
                "Failed to load plot config from %s (%s). Falling back to config_storage.py",
                resolved_path,
                ex,
            )

    _ACTIVE_PLOT_CONFIG = Gen7V1_V2
    _ACTIVE_PLOT_CONFIG_SOURCE = "can_inplot/c_data_storage/config_storage.py"
    return _ACTIVE_PLOT_CONFIG, _ACTIVE_PLOT_CONFIG_SOURCE



def get_plot_config() -> Dict:
    return _ACTIVE_PLOT_CONFIG


def get_stream_plot_config(stream_name: str) -> Dict:
    config = get_plot_config()
    if stream_name in config:
        return config[stream_name]

    normalized_stream = str(stream_name or "").upper()
    if "DETECTION" in normalized_stream and "DETECTION_STREAM" in config:
        detection_config = config["DETECTION_STREAM"]
        bordnet_config = {}
        for signal_name, signal_config in detection_config.items():
            if not isinstance(signal_config, dict):
                continue
            signal_config = dict(signal_config)
            plot_types = signal_config.get("plot_types")
            if not plot_types:
                plot_types = ["scatter_plot"]
            signal_config["plot_types"] = list(plot_types)
            bordnet_config[signal_name] = signal_config
        if "timestamp" not in bordnet_config:
            bordnet_config["timestamp"] = {
                "aliases": ["timestamp"],
                "call": ["Timestamp"],
                "plot_types": ["scatter_plot"],
                "unit": "timestamp",
            }
        return bordnet_config

    if "HEADER" in normalized_stream and "HEADER_STREAM" in config:
        header_config = config["HEADER_STREAM"]
        bordnet_header_config = {}
        for signal_name, signal_config in header_config.items():
            if not isinstance(signal_config, dict):
                continue
            signal_config = dict(signal_config)
            plot_types = signal_config.get("plot_types")
            if not plot_types:
                plot_types = ["scatter_plot"]
            signal_config["plot_types"] = list(plot_types)
            bordnet_header_config[signal_name] = signal_config
        if "timestamp" not in bordnet_header_config:
            bordnet_header_config["timestamp"] = {
                "aliases": ["timestamp"],
                "call": ["Timestamp"],
                "plot_types": ["scatter_plot"],
                "unit": "timestamp",
            }
        return bordnet_header_config

    return {}


def resolve_signal_name(stream_name: str, item_name: str, stream_config: Optional[Dict] = None) -> Optional[str]:
    config = stream_config if stream_config is not None else get_stream_plot_config(stream_name)
    item_text = str(item_name or "")
    item_lower = item_text.lower()

    if item_text in config:
        return item_text

    for signal_name, signal_config in config.items():
        if not isinstance(signal_config, dict):
            continue
        aliases = {str(alias).lower() for alias in signal_config.get("aliases", [])}
        if item_lower in aliases:
            return signal_name

    if item_lower.startswith("timestamp_") and "timestamp" in config:
        return "timestamp"

    bordnet_prefixes = {
        "det_range": "ran",
        "det_range_velocity": "vel",
        "det_azimuth": "theta",
        "det_elevation": "phi",
    }
    base_name = re.sub(r"_\d{3}$", "", item_lower)
    canonical_name = bordnet_prefixes.get(base_name)
    if canonical_name in config:
        return canonical_name

    return None



def get_plot_config_source() -> str:
    return _ACTIVE_PLOT_CONFIG_SOURCE
