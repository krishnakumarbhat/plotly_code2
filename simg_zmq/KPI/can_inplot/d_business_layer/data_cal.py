import numpy as np
from can_inplot.e_presentation_layer.plotly_visualization import PlotlyCharts
import logging
...
class DataCalculations:
    ...
    def set_signal_config(self, signal_config) -> None:
        """Set the active per-signal config (enables x_axis/y_label overrides)."""
        self.signal_config = signal_config if isinstance(signal_config, dict) else {}

    def _resolve_x(self, signal_name, si, i_vals, data_dict):
        """Resolve the x-axis values and label for a plot.

        Config keys (per signal in config.json):
            x_axis: "scan_index" (default) or the name/alias of another
                    signal in the same stream to use as x values.
            x_label: optional custom x-axis title.
        The DataPrep layer stashes the alternate x signal as XI in
        data_dict when configured. Falls back to scan index on any mismatch.
        """
        x_axis = (self.signal_config or {}).get("x_axis")
        if not x_axis or str(x_axis).strip().lower() in {"scan_index", "scanindex", "si"}:
            return si, "ScanIndex"
        xi = data_dict.get("XI")
        if xi is None:
            logging.warning(
                "x_axis %r requested for %s but no x data resolved; falling back to ScanIndex",
                x_axis, signal_name,
            )
            return si, "ScanIndex"
        try:
            if len(np.asarray(xi)) < len(np.asarray(i_vals)):
                raise ValueError("shorter than signal")
        except Exception:
            return si, "ScanIndex"
        label = (self.signal_config or {}).get("x_label") or str(x_axis)
        return xi, label
    ...
    def scan_continuity_plot(self, signal_name, data_dict, shared_data, lock):
        """
        Scan continuity / dropped-frame analysis (automotive radar validation).

        Analyzes the stream's scan-index sequence and reports:
          - duplicate scan indices (retransmitted rows)
          - out-of-order scans
          - dropped scans (gaps in the expected +1 sequence)
        Rendered as a gap-size bar chart with a summary annotation.
        ...
        si_raw = data_dict.get("SI")
        si = np.asarray([] if si_raw is None else si_raw, dtype=float)
        si = si[np.isfinite(si)]
        if si.size < 2:
            return None, None

        uniq, counts = np.unique(si, return_counts=True)
        duplicates = int(np.sum(counts[counts > 1] - 1))
        diffs = np.diff(uniq)
        out_of_order = int(np.sum(diffs < 0))
        gaps = diffs[diffs > 1]
        dropped = int(np.sum(gaps - 1)) if gaps.size else 0

        gap_values, gap_counts = (np.array([]), np.array([]))
        if gaps.size:
            gap_values, gap_counts = np.unique(gaps.astype(int), return_counts=True)
        ...
        if gaps.size:
            fig.add_trace(go.Bar(
                ...
                y=gap_counts,
                marker_color="#d62728",
                name="gap occurrences",
            ))
        total_scans = int(uniq.size)
        continuity_pct = round(100.0 * (total_scans - dropped) / max(total_scans, 1), 2)
        fig.add_annotation(
            text=(f"scans: {total_scans} | dropped: {dropped} | duplicates: {duplicates} | "
                  f"out-of-order: {out_of_order} | continuity: {continuity_pct}%"),
            xref="paper", yref="paper", x=0.5, y=1.12, showarrow=False,
            font=dict(size=13, color="#17324a"),
        )
        fig.update_layout(
            title=f"Scan Continuity — {signal_name}",
            xaxis_title="scan-index gap size (rows skipped + 1)",
            yaxis_title="occurrences",
            bargap=0.1,
        )
        return f"scan_continuity_{signal_name}", fig
    ...
    def scatter_plot(self, signal_name, data_dict, shared_data, lock):
        ...
        x_vals, x_label = self._resolve_x(signal_name, si, i_vals, data_dict)
        fig_id = f"scatter_fig_{signal_name}"
        fig = PlotlyCharts.scatter_plot(
            x_vals,
            i_vals,
            ...
            "IN/OUT",
            xaxis_title=x_label,
            yaxis_title=(self.signal_config or {}).get("y_label"),
        )
        return fig_id, fig
