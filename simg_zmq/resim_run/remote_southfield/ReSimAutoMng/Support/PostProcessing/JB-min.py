...
def collect_jobouts():
    ...
    paths=[p for p in file.readlines() if p.strip()]
    file.close()

    # Filenames look like ".../<task_id>/<job_id>_<task_id>.out". When the
    # ReSim submission is split into multiple sbatch array jobs (task count
    # >1000), each chunk's array restarts at task id 1, so the same task_id
    # (e.g. "_18.") shows up once per chunk pointing at a *different* job_id.
    # Sorting/deduping on task_id alone (as before) made every chunk collapse
    # onto the first match, duplicating one job's log while dropping the
    # others'. Sort on (task_id, job_id) instead so every distinct file
    # (unique job_id+task_id pair) is kept exactly once.
    def sort_key(path):
        stem = path.strip().split('/')[-1].split('.')[0]
        job_id, _, task_id = stem.rpartition('_')
        return (int(task_id), int(job_id) if job_id.isdigit() else job_id)

    jobouts = sorted(paths, key=sort_key)
...
def update_latched(message, file, latchedScan, missedScan, currentInput):
    def update_data(overall, data):
        for key in data.keys():
            # Safely handle both int and str values
            data_val = data[key]
            if isinstance(data_val, str) and data_val != 'NA':
                try:
                    data_val = int(data_val.strip())
                except (ValueError, AttributeError):
                    data_val = 0
            elif not isinstance(data_val, int) and data_val != 'NA':
                data_val = 0
            
            if overall[key] == 'NA':
                overall[key] = data_val
            else:
                # Ensure both operands are ints
                try:
                    overall_val = int(overall[key]) if isinstance(overall[key], str) else (overall[key] if isinstance(overall[key], int) else 0)
                    data_val = int(data_val) if isinstance(data_val, str) else (data_val if isinstance(data_val, int) else 0)
                    overall[key] = overall_val + data_val
    ...
    for sensor in  sensorScan[1:]:
        info=sensor.split(']:')
        try:
            sensor_key = info[0].strip() if len(info) > 0 else 'NA'
            sensor_val = int(info[1].strip()) if len(info) > 1 else 0
            if sensor_key in scancnt:
                scancnt[sensor_key] = sensor_val
    ...
    if latchedScanCnt in message :
        ...
        for value in scancnt.values():
            if value != 'NA':
                try:
                    val_int = int(value) if isinstance(value, str) else (value if isinstance(value, int) else 0)
                    scanLogmissed += val_int
                except (ValueError, TypeError):
...
# writing data to xlsx file
    ...
    for detail in all_details:
        ...
        if line >= currLine and versionCaptured == True:
            ...
            if content[2] == ' OOM     ':
                sessionOOM+=1
                # OOM logs must count regardless of which stage resimComp was in when killed
                if content[12] == ' False  ': logsOOM += failed_logs
                else: logsCrashed += failed_logs
            if content[2] == ' CALIB_P ' or content[2] == ' CALIB_F ' :
                if failed_logs == 0:
                    if content[13] != ' Done  ': content[2] = 'Partial'
                    else: content[2]='Done'
                ...
                for val in content[1:]:
                    if 'NA' not in val:
                        try:
                            ...
                            pass  # Skip values that can't be converted to int (e.g., sensor versions)
