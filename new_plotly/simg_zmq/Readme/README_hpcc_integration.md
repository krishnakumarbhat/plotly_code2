...
5. Slurm-backed jobs stay `PENDING` until a compute-host tmux pane writes a `START` marker; only then do history and job-log views flip them to `RUNNING`.
6. RAG is an on-demand service; when launched it publishes its compute-node host back into the runtime registry instead of assuming localhost.
7. Hyperlink is integrated directly into `main_html`, reuses the same session context, and keeps AI video description offline-only.
...
- `hpcc_main.py`
  ...
  - process supervisor
  - runtime log writer
  - Slurm + tmux launcher writer
  - status reconciliation from `launcher.log`, `runtime_console.log`, and exit markers
...
For Slurm-backed KPI runs it also derives user-facing state from run artifacts:

- `PENDING` while Slurm has not allocated compute yet
- `RUNNING` once `[MAIN]`, `[UDP]`, or `[INTERACTIVE] START` appears in `runtime_console.log`
- `COMPLETED` or `FAILED` from exit markers and broker return codes
...
- `simg_sh_hpcc/main_hpcc.sh` as the login-node entrypoint
- `simg_sh_hpcc/run_hpcc_stack.sh` as the cluster-side launcher wrapper

Only `main_html` and Hyperlink should remain active on the login node. KPI and Interactive Plot work is expected to go through Slurm from the broker or through the tmux-backed direct wrappers in `simg_sh_hpcc/kpi/`. The deployed bundle bind-mounts `bundle_src/main_html` and `bundle_src/Hyperlink_tool`, so targeted source uploads take effect after a restart without rebuilding `main_html.simg`.
...
For Slurm-backed KPI runs the runtime now distinguishes scheduler queueing from active compute:

1. `PENDING`: the broker process exists and Slurm has accepted the request, but compute has not been allocated yet.
2. `RUNNING`: a tmux pane on the compute allocation has written a `START` marker into `runtime_console.log`.
3. `COMPLETED` or `FAILED`: the per-pane exit marker or broker return code has been written.

This prevents the UI from showing queued jobs as if they were already executing.
