...
cmds = [
    "sinfo -o '%P %a %l %D %N' 2>&1",
    "squeue -u ouymc2 2>&1",
    "sacctmgr show assoc user=ouymc2 format=account,partition,qos 2>&1",
]
for c in cmds:
    print('=== ', c, ' ===')
    stdin, stdout, stderr = client.exec_command(f"bash -lc '{setup} {c}'", timeout=25)
