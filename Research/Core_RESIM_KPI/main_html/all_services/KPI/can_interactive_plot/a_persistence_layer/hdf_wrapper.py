...
from sensor_matcher import canonical_sensor_id
...
class HdfAttrReader:
    ...
    def _canonical_sensor_id(self, sensor_id: str) -> str:
        # Producers write sensor names with arbitrary prefixes and spaces
        # (e.g. "CEER_ FL", "MCIP_FL", "SRR_FL", "FL").  Reduce every family
        # to the stable position-based CEER_* id so input and output files —
        # even from different producer builds — share one canonical key.
        cleaned = "".join(str(sensor_id).upper().split())
        if cleaned in self.SENSOR_ALIASES:
            return self.SENSOR_ALIASES[cleaned]
        return canonical_sensor_id(sensor_id)
