#!/bin/bash
# ==============================================================================
# KPI Tool - Slurm Submission Script
# ==============================================================================
# Usage:
#   ./run_kpi.sh json <KPIPlot.json> [html_dir]
#   ./run_kpi.sh hdf <input.h5> <output.h5> [html_dir]
#   ./run_kpi.sh --local json <KPIPlot.json> [html_dir]  (local with progress)
# ==============================================================================
...
print_usage() {
    echo "KPI Tool - HPC/Local Submission Script"
    echo ""
    echo "Usage:"
    echo "  JSON Batch Mode (SLURM):"
    echo "    $0 json <KPIPlot.json> [html_dir]"
    echo ""
    echo "  HDF Pair Mode (SLURM):"
    echo "    $0 hdf <input.h5> <output.h5> [html_dir]"
    echo ""
    echo "  Local Mode (with progress tracking):"
    echo "    $0 --local json <KPIPlot.json> [html_dir]"
    ...
    echo ""
    echo "Examples:"
    echo "  $0 json KPIPlot.json /output/html_db"
    echo "  $0 --local json KPIPlot.json /output/html_db"
    echo "  $0 hdf input.h5 output.h5 /output/html_db"
}
...
if [[ "$LOCAL_MODE" == false ]] && [[ ! -f "$SIMG_FILE" ]]; then
    ...
    echo "  apptainer build --fakeroot kpi.simg Singularity_KPI.def"
    exit 1
fi
...
case "$MODE" in
    json)
        if [[ -z "$1" ]]; then
            echo -e "${RED}Error: JSON file path required${NC}"
            print_usage
            exit 1
        fi
        ...
        echo -e "${GREEN}=== KPI Tool - JSON Batch Mode ===${NC}"
        ...
        SINGULARITY_CMD="singularity run $BIND_MOUNTS $SIMG_FILE json \"$JSON_FILE\" \"$HTML_DIR\""
        ;;
    ...
    hdf)
        ...
        echo -e "${GREEN}=== KPI Tool - HDF Pair Mode ===${NC}"
        ...
        BIND_MOUNTS=$(get_bind_mounts)
        SINGULARITY_CMD="singularity run $BIND_MOUNTS $SIMG_FILE hdf \"$INPUT_HDF\" \"$OUTPUT_HDF\" \"$HTML_DIR\""
        ;;
        
    *)
        echo -e "${RED}Error: Unknown mode '$MODE'. Use 'json' or 'hdf'${NC}"
        print_usage
        exit 1
        ;;
esac
...
if [[ "$LOCAL_MODE" == true ]]; then
    ...
    # Count pairs if JSON mode
    if [[ "$MODE" == "json" ]]; then
        TOTAL_PAIRS=$(python3 -c "import json; data=json.load(open('$JSON_FILE')); print(len(data))" 2>/dev/null || echo "?")
        echo "Total HDF pairs: $TOTAL_PAIRS"
    else
        TOTAL_PAIRS=1
    fi
    ...
    if [[ -n "$RUNTIME" ]] && [[ -f "$SIMG_FILE" ]]; then
        ...
        echo ""
        $RUNTIME run $BIND_MOUNTS "$SIMG_FILE" $MODE "$@" 2>&1 | \
            while IFS= read -r line; do
                echo "$line"
                if [[ "$line" == *"Processing"* ]] || [[ "$line" == *"HDF Pair"* ]]; then
                    ((CURRENT++)) || CURRENT=1
                    if [[ "$TOTAL_PAIRS" != "?" ]]; then
        ...
        cd "$SCRIPT_DIR/../tools"
        if [[ "$MODE" == "json" ]]; then
            python3 -m KPI.kpi_server "$JSON_FILE" "$HTML_DIR" 2>&1 | \
                while IFS= read -r line; do
                    echo "$line"
            ...
            python3 -m KPI.kpi_server "$INPUT_HDF" "$OUTPUT_HDF" "$HTML_DIR"
        fi
    fi
    ...
    echo ""
    echo -e "${GREEN}========================================${NC}"
    echo -e "${GREEN}KPI processing completed!${NC}"
...
JOB_ID=$(srun --parsable --exclusive -N 1 -n 1 \
    ...
    --output="$LOG_DIR/kpi_%j.out" \
    --error="$LOG_DIR/kpi_%j.err" \
    --job-name="kpi" \
...
if [[ $RC -ne 0 ]]; then
    ...
    echo "See logs in: $LOG_DIR (kpi_${JOB_ID}.out/.err)"
    exit $RC
fi

echo -e "${GREEN}Job completed successfully!${NC}"
echo "  Job ID: $JOB_ID"
echo "  Logs:   $LOG_DIR/kpi_${JOB_ID}.out (.err)"
