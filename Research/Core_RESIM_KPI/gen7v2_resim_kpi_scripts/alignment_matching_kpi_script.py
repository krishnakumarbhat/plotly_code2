...
class Alignment_Vars:
    ...
    def _initialize(self):
        ...
        self.raw_speed_real_list = []
        self.raw_speed_sim_list = []
        self.estimated_speed_real_list = []
        self.estimated_speed_sim_list = []
        self.raw_yawrate_real_list = []
        self.raw_yawrate_sim_list = []
        self.estimated_yawrate_real_list = []
        self.estimated_yawrate_sim_list = []        
        self.vcs_sensor_long_posn_real_list = []
        self.vcs_sensor_long_posn_sim_list = []
        self.vcs_sensor_lat_posn_real_list = []
        self.vcs_sensor_lat_posn_sim_list = []
        self.sensor_height_over_ground_real_list = []
        self.sensor_height_over_ground_sim_list = []
        self.vwrapping_real_list = []
        self.vwrapping_sim_list = []
        self.vacs_boresight_roll_nominal_real_list = []
        self.vacs_boresight_roll_nominal_sim_list = []
#################################
...
def main(veh_csv_list, sim_csv_list) -> bool:
    ...
    cols_of_interest = [
        ...
        "raw_yawrate",
        "estimated_yawrate",
        "vcs_sensor_long_posn",
        "vcs_sensor_lat_posn",
        "sensor_height_over_ground",
        "vwrapping",
        "vacs_boresight_roll_nominal"
    ]
    ...
    final_df['scan_index_diff'] = final_df['scan_index'].diff() != 1
    
    alignment_vars.scan_index_list = final_df["scan_index"].values.tolist()
    alignment_vars.scan_index_diff_list = final_df["scan_index_diff"].values.tolist()
    ...
    alignment_vars.raw_speed_real_list = final_df["raw_speed_real"].values.tolist()
    alignment_vars.raw_speed_sim_list = final_df["raw_speed_sim"].values.tolist()
    alignment_vars.estimated_speed_real_list = final_df["estimated_speed_real"].values.tolist()
    alignment_vars.estimated_speed_sim_list = final_df["estimated_speed_sim"].values.tolist()
    alignment_vars.raw_yawrate_real_list = final_df["raw_yawrate_real"].values.tolist()
    alignment_vars.raw_yawrate_sim_list = final_df["raw_yawrate_sim"].values.tolist()
    alignment_vars.estimated_yawrate_real_list = final_df["estimated_yawrate_real"].values.tolist()
    alignment_vars.estimated_yawrate_sim_list = final_df["estimated_yawrate_sim"].values.tolist()
    alignment_vars.vcs_sensor_long_posn_real_list = final_df["vcs_sensor_long_posn_real"].values.tolist()
    alignment_vars.vcs_sensor_long_posn_sim_list = final_df["vcs_sensor_long_posn_sim"].values.tolist()
    alignment_vars.vcs_sensor_lat_posn_real_list = final_df["vcs_sensor_lat_posn_real"].values.tolist()
    alignment_vars.vcs_sensor_lat_posn_sim_list = final_df["vcs_sensor_lat_posn_sim"].values.tolist()
    alignment_vars.sensor_height_over_ground_real_list = final_df["sensor_height_over_ground_real"].values.tolist()
    alignment_vars.sensor_height_over_ground_sim_list = final_df["sensor_height_over_ground_sim"].values.tolist()
    alignment_vars.vwrapping_real_list = final_df["vwrapping_real"].values.tolist()
    alignment_vars.vwrapping_sim_list = final_df["vwrapping_sim"].values.tolist()
    alignment_vars.vacs_boresight_roll_nominal_real_list = final_df["vacs_boresight_roll_nominal_real"].values.tolist()
    alignment_vars.vacs_boresight_roll_nominal_sim_list = final_df["vacs_boresight_roll_nominal_sim"].values.tolist()
...
def plot_stats():
    ...
    #############################################################################################################
    if config.ALIGN_ADDITIONAL_PLOTS:
        num_rows = 15
        total_height = 5000
        vertical_spacing_used = 0.02
    else:
        num_rows =  3    
        total_height = 1000
        vertical_spacing_used = 0.1
        
    fig_line = sp.make_subplots(rows=num_rows, cols=1, horizontal_spacing=0.04, vertical_spacing=vertical_spacing_used)

    # Manually add traces for line plots    
    row_num = 1
    fig_line.add_trace(go.Scatter(x=alignment_vars.scan_index_list, y=alignment_vars.scan_index_diff_list, mode="lines", name="Scan Index Diff", line=dict(color="blue")), row=row_num, col=1)
    fig_line.update_yaxes(title_text="Scan Index Diff", row=row_num, col=1)
    ...
    if config.ALIGN_ADDITIONAL_PLOTS:
        ...
        row_num = row_num + 1
        fig_line.add_trace(go.Scatter(x=alignment_vars.scan_index_list, y=alignment_vars.raw_speed_real_list, mode="lines", name="Veh Raw Speed", line=dict(color="blue")), row=row_num, col=1)
        fig_line.add_trace(go.Scatter(x=alignment_vars.scan_index_list, y=alignment_vars.raw_speed_sim_list, mode="lines", name="Sim Raw Speed", line=dict(color="red")), row=row_num, col=1)
        fig_line.update_yaxes(title_text="Raw Speed", row=row_num, col=1)
        fig_line.update_xaxes(title_text="Scan Index", row=row_num, col=1)

        row_num = row_num + 1
        fig_line.add_trace(go.Scatter(x=alignment_vars.scan_index_list, y=alignment_vars.estimated_speed_real_list, mode="lines", name="Veh Estimated Speed", line=dict(color="blue")), row=row_num, col=1)
        fig_line.add_trace(go.Scatter(x=alignment_vars.scan_index_list, y=alignment_vars.estimated_speed_sim_list, mode="lines", name="Sim Estimated Speed", line=dict(color="red")), row=row_num, col=1)
        fig_line.update_yaxes(title_text="Estimated Speed", row=row_num, col=1)
        fig_line.update_xaxes(title_text="Scan Index", row=row_num, col=1)

        row_num = row_num + 1
        fig_line.add_trace(go.Scatter(x=alignment_vars.scan_index_list, y=alignment_vars.raw_yawrate_real_list, mode="lines", name="Veh Raw Yawrate", line=dict(color="blue")), row=row_num, col=1)
        fig_line.add_trace(go.Scatter(x=alignment_vars.scan_index_list, y=alignment_vars.raw_yawrate_sim_list, mode="lines", name="Sim Raw Yawrate", line=dict(color="red")), row=row_num, col=1)
        fig_line.update_yaxes(title_text="Raw Yawrate", row=row_num, col=1)
        fig_line.update_xaxes(title_text="Scan Index", row=row_num, col=1)

        row_num = row_num + 1
        fig_line.add_trace(go.Scatter(x=alignment_vars.scan_index_list, y=alignment_vars.estimated_yawrate_real_list, mode="lines", name="Veh Estimated Yawrate", line=dict(color="blue")), row=row_num, col=1)
        fig_line.add_trace(go.Scatter(x=alignment_vars.scan_index_list, y=alignment_vars.estimated_yawrate_sim_list, mode="lines", name="Sim Estimated Yawrate", line=dict(color="red")), row=row_num, col=1)
        fig_line.update_yaxes(title_text="Estimated Yawrate", row=row_num, col=1)
        fig_line.update_xaxes(title_text="Scan Index", row=row_num, col=1)

        row_num = row_num + 1
        fig_line.add_trace(go.Scatter(x=alignment_vars.scan_index_list, y=alignment_vars.vwrapping_real_list, mode="lines", name="Veh Vwrapping", line=dict(color="blue")), row=row_num, col=1)
        fig_line.add_trace(go.Scatter(x=alignment_vars.scan_index_list, y=alignment_vars.vwrapping_sim_list, mode="lines", name="Sim Vwrapping", line=dict(color="red")), row=row_num, col=1)
        fig_line.update_yaxes(title_text="Vwrapping", row=row_num, col=1)
        fig_line.update_xaxes(title_text="Scan Index", row=row_num, col=1)

        row_num = row_num + 1
        fig_line.add_trace(go.Scatter(x=alignment_vars.scan_index_list, y=alignment_vars.vacs_boresight_roll_nominal_real_list, mode="lines", name="Veh Roll", line=dict(color="blue")), row=row_num, col=1)
        fig_line.add_trace(go.Scatter(x=alignment_vars.scan_index_list, y=alignment_vars.vacs_boresight_roll_nominal_sim_list, mode="lines", name="Sim Roll", line=dict(color="red")), row=row_num, col=1)
        fig_line.update_yaxes(title_text="Roll", row=row_num, col=1)
        fig_line.update_xaxes(title_text="Scan Index", row=row_num, col=1)

        row_num = row_num + 1
        fig_line.add_trace(go.Scatter(x=alignment_vars.scan_index_list, y=alignment_vars.vcs_sensor_long_posn_real_list, mode="lines", name="Veh VCS Sensor Long Pos", line=dict(color="blue")), row=row_num, col=1)
        fig_line.add_trace(go.Scatter(x=alignment_vars.scan_index_list, y=alignment_vars.vcs_sensor_long_posn_sim_list, mode="lines", name="Sim VCS Sensor Long Pos", line=dict(color="red")), row=row_num, col=1)
        fig_line.update_yaxes(title_text="VCS Sensor Long Pos", row=row_num, col=1)
        fig_line.update_xaxes(title_text="Scan Index", row=row_num, col=1)

        row_num = row_num + 1
        fig_line.add_trace(go.Scatter(x=alignment_vars.scan_index_list, y=alignment_vars.vcs_sensor_lat_posn_real_list, mode="lines", name="Veh VCS Sensor Lat Pos", line=dict(color="blue")), row=row_num, col=1)
        fig_line.add_trace(go.Scatter(x=alignment_vars.scan_index_list, y=alignment_vars.vcs_sensor_lat_posn_sim_list, mode="lines", name="Sim VCS Sensor Lat Pos", line=dict(color="red")), row=row_num, col=1)
        fig_line.update_yaxes(title_text="VCS Sensor Lat Pos", row=row_num, col=1)
        fig_line.update_xaxes(title_text="Scan Index", row=row_num, col=1)

        row_num = row_num + 1
        fig_line.add_trace(go.Scatter(x=alignment_vars.scan_index_list, y=alignment_vars.sensor_height_over_ground_real_list, mode="lines", name="Veh Sensor Height Over Ground", line=dict(color="blue")), row=row_num, col=1)
        fig_line.add_trace(go.Scatter(x=alignment_vars.scan_index_list, y=alignment_vars.sensor_height_over_ground_sim_list, mode="lines", name="Sim Sensor Height Over Ground", line=dict(color="red")), row=row_num, col=1)
        fig_line.update_yaxes(title_text="Sensor Height Over Ground", row=row_num, col=1)
        fig_line.update_xaxes(title_text="Scan Index", row=row_num, col=1)

    # Update layout for bar plot
    fig_line.update_layout(height=total_height, width=1400, title_text="Misalignment v/s scanindex", showlegend=True)
    ...
    misalign_line_plot_html = pio.to_html(fig_line, full_html=False, include_plotlyjs=False)
    #############################################################################################################
    ...
    misalign_diff_plot_html = pio.to_html(fig_line, full_html=False, include_plotlyjs=False)
    #############################################################################################################
