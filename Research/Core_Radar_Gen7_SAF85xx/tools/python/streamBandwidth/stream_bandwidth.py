...
REC_PAYLOAD = 1412  # Bytes

"""CDC max chunks/frame calculation"""
SPBB_MAX_CDC_DBIN_COUNT_PER_FRAME = 5016  # Enabled CDC_DBins
TOTAL_CDC_RECORDS_PER_FRAME = 35  # CDC records per one frame
PARTIAL_FRAME = 1  # Partial Data frame
...
for file in args.input_files.readlines():
    ...
    if ("str014" in filename) or ("str046" in filename):
        ...
        with open(file, "r") as file_1:
            for line in file_1:
                stream_chunks = int(stream_total_chunks(int(line)))
                size[filename] = [
                    int(line),
                    int(stream_chunks),
