...
def parse_html_report(file_path):
    ...
    skipped_tests = 0
    errors_tests = 0
    # Regex to extract overall test results
...
def standardize_module_name(name):
    """Removing numbers unwante characters after the first space & Keeping only letters and underscores."""
    # Remove the `::test_` prefix
    name = re.sub(r"^::test_", "", name, flags=re.IGNORECASE)
    # Remove any numbers and trailing underscores
    name = re.sub(r"_\d+|\d+|_$", "", name)
    # Keep only the first part before spaces
    name = name.split()[0]
    # Capitalize and format remaining parts
    name_parts = name.split("_")
    standardized_name = "_".join(part.capitalize() for part in name_parts if part)
    return standardized_name


def extract_module_names_from_html(file_path):
    """
    Extract unique module names dynamically from the test cases in the HTML report.
    """
    ...
    for row in rows:
        ...
        if test_case_name_td:
            test_case_name = test_case_name_td.text.strip()
            match = module_pattern.search(test_case_name)
            if match:
                raw_module_name = match.group(1)
                standardized_module_name = standardize_module_name("::test_" + raw_module_name)
                module_names.add(standardized_module_name)
...
def extract_module_results_from_html(file_path, module_names):
    ...
    rows = soup.select("table#results-table tbody tr")
    # Define a regex pattern to match valid module names
    module_pattern = re.compile(r"::test_([a-zA-Z_][a-zA-Z_\d]*)", re.IGNORECASE)
    for row in rows:
        test_case_name_td = row.find("td", {"class": "col-name"})
        ...
        # Apply regex to extract the module name from the test case name
        match = module_pattern.search(test_case_name)
        if match:
...
if __name__ == "__main__":
    ...
    if os.path.exists(report_path):
        ...
        print(f"Overall Failed={failed_tests}")
        print(f"Overall Skipped={skipped_tests}")
        print(f"Overall Errors={errors_tests}")

        metric_name = f"advradar.gen7_v2.{test_type}.testcases"

        # Send overall test results to Datadog
        dataDogInst.sendGaugeMetric(
            metric_name,
            passed_tests,
            tags=["gen7_v2_env:test", "Type:passed", f"Variant:{variant}"],
        )
        dataDogInst.sendGaugeMetric(
            metric_name,
            failed_tests,
            tags=["gen7_v2_env:test", "Type:failed", f"Variant:{variant}"],
        )
        dataDogInst.sendGaugeMetric(
            metric_name,
            skipped_tests,
            tags=["gen7_v2_env:test", "Type:skipped", f"Variant:{variant}"],
        )
        dataDogInst.sendGaugeMetric(
            metric_name,
            errors_tests,
            tags=["gen7_v2_env:test", "Type:errors", f"Variant:{variant}"],
        )
        # Extract module names dynamically from the HTML report
        module_names = extract_module_names_from_html(report_path)
        ...
        for module, results in module_results.items():
            ...
            error = results["Error"]
            print(f"{module} - Passed: {passed}, Failed: {failed}, Error: {error}")
            dataDogInst.sendGaugeMetric(
                metric_name,
                passed,
                tags=[
