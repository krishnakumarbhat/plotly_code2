...
parser.add_argument(
    "-o", "--output", type=str, required=True, help="The output flash image file name"
)
parser.add_argument(
...
parser.add_argument("-e", "--entry-address", type=int)
parser.add_argument("-s", "--start-address", type=int)
parser.add_argument("-g", "--min-gap", type=int, default=16)
...
parser.add_argument(
    "-r", "--ram-image", type=str, required=False, help="The output merged RAM image file name"
)
...
merged_image.fill(max_words=args.min_gap)
...
flash_image.execution_start_address = args.entry_address
flash_image.header = "Dynamically loaded flash image " + args.pattern

with open(args.output, "w") as f:
    f.write(flash_image.as_srec())

if args.ram_image is not None:
    merged_image.execution_start_address = args.entry_address
    merged_image.header = "Merged RAM image " + args.pattern

    with open(args.ram_image, "w") as f:
        f.write(merged_image.as_srec())
