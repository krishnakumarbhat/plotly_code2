...
register_toolchains(
 ...
 - `gen7`: https://gitgerrit.asux.aptiv.com/a/plugins/gitiles/ADVRADAR_AWR294X/+archive/refs/tags/v10.0.100.tar.gz
...
- Copy the radar_stream_decoder.dll from `10028634_01_SRR_RESIM_Release/RESIM_SIL/DECODER_DLL` to `10028634_01_SRR_RESIM_Release/RESIM_SIL/GEN7_SIL_ENGINE`
...
We have duplicated typedefs in `streams/` because using `rdd_stream.h` from `@gen7//software/app/common:UDPLoggingStreamHeaders` causes struct redefinition errors.
...
- Re-check `populate_ipc_data()` function
- Update SMC and USC jfrog links in `WORKSPACE`
- Update all stream population to be in sync with AWR
  - Compare `udp` folder with `stream_handler` folder from AWR.
  - Visualize in both Orcas live mode and by opening the output mf4 file.
- Update Tracker wrapper files
- Update Gen7_SiL
  - Compare and update `WORKSPACE`
  - Update `building_block` folder in `emb_lib`
  - Update `spbb`, `afbb`, `idbb`, `dabb`, `rcbb` and `sabb` links
  - emb_lib/building_block should be in sync with @gen7_sil building_block
  - The macro `MAX_CDC_TDC_DBIN_COUNT_PER_FRAME` has to maintained at value `5120` in `\building_block\cdc_tdc_process\cdc_tdc_process_cfg.h`
  - Update `rsp_emb_lib`
    - Check `get_sil_input()` and `populate_rsp_output()`.
    - Check `rsp_sil_wrapper.h` against `sil_wrapper.h` from `@gen7_sil`
