...
@pytest.mark.WI("WI_348734")
@pytest.mark.description("Qualification test validation for temperature sensor consistency check - Negative Test Case")
def test_FRONT_END_MANAGEMENT(Power, T32_R5A, T32_C66, Report):
    ...
    var_names = [
        ...
        "MMIC_Stream.MMIC_Data.MMIC_Fault_Status.MMIC_temp_Fault.MMIC_Consistency_Check_Fault",
        "MMIC_Reconfig",
        "XCP_MMIC_FIT_Enable"
    ]

    # Add watches and read initial values for all variables in a single loop
    var_values = []
    for var in var_names:
        ...
        print(f"{var} = {value}")
        time.sleep(0.5)
    temp_2, temp_3, temp_4, temp_5 = var_values

    print("Overwriting XCP_MMIC_FIT_Enable to XCP_MMIC_Temp_Consistency_FIT")
    T32_R5A.cmd("Var.set %e %sp XCP_MMIC_FIT_Enable = 8")
    time.sleep(2)
    ...
    for var in var_names:
        value = T32_R5A.read_var(var)
        updated_var_values.append(value)
        print(f"{var} = {value}")
        time.sleep(0.5)
    temp_2, temp_3, temp_4, temp_5 = updated_var_values

    print("")
    print("-----------------------------------WI-348734 :END-------------------------------------")
    time.sleep(5)
    if temp_2 == 1 and temp_3 == 170 and flag:
        print("Passed")
        assert True
    else:
        print("failed")
