...
def test_Measurement_Monitor(Power, T32_R5A, T32_C66, Report):
    ...
    assert ((TEMP_LOW[0] == 2) and (TEMP_LOW[1] == 1)), "TEMP LOW Conditions were Not Satisfied. WI-279770, Test Case got Failed"
    print("TEMP LOW Conditions were Satisfied. CASE-1 PASS.")
    ...
    if len(fault_name) > 0:
        for i in range(0, len(fault_name)):
            ...
            if fault_counter[i] != 0:
                ...
                low_counter += 1
                print()
        assert (low_counter == 0), "Few faults are set after voltage transitioned from 6.9V to 12V. WI-279770, Test Case got Failed. Check the HTML Report."
    ...
    TEMP_CURRENT[0] = T32_R5A.read_var(temp[0])
    print("Checking ", temp[0], " = ", T_1[TEMP_CURRENT[0]])
    assert ((TEMP_HIGH[0] == 3) and (TEMP_HIGH[2] == 1)), "TEMP HIGH Conditions were Not Satisfied. WI-279770, Test Case got Failed"
    print("TEMP HIGH Conditions were Satisfied. CASE-3 PASS.")
    ...
    if len(fault_name) > 0:
        for i in range(0, len(fault_name)):
            ...
            if fault_counter[i] != 0:
                ...
                high_counter += 1
                print()
        assert (high_counter == 0), "Few faults are set after voltage transitioned from 17V to 12V. WI-279770, Test Case got Failed. Check the HTML Report."
    ...
    print("============> WI-279770, Test Case got Passed")
