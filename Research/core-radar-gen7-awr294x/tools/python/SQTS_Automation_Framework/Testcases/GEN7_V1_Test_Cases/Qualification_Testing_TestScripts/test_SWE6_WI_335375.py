...
@pytest.mark.WI("WI-335375")
@pytest.mark.description("Qualification test validation fr RF Heakth Malfunction Fault - Positive Test Case")
def test_FRONT_END_MANAGEMENT(Power, T32_R5A, T32_C66, Report):
    ...
    try:
        ...
        print("  Setting breakpoint at Populate_MMIC_Stream")
        T32_R5A.set_breakpoint(command)
        time.sleep(0.5)
        T32_R5A.cmd("Go")
        time.sleep(1)
        if T32_R5A.get_run_state() == 2:
            print("  Breakpoint hit at start of Populate_MMIC_Stream")
        else:
            print("  Breakpoint NOT hit at start of Populate_MMIC_Stream")
    except Exception as e:
        print(f"  Error: {e}")
    # Overwrite RFHealthPercentage to 89
    try:
        print("  Setting RFHealthPercentage = 89")
        T32_R5A.write_var('RFHealthPercentage', 89)
        time.sleep(1)
    except Exception as e:
        print(f"  Error: {e}")
    ...
    try:
        ...
        print("  Setting breakpoint at end of Populate_MMIC_Stream")
        T32_R5A.set_breakpoint(command)
        time.sleep(0.5)
        T32_R5A.cmd("Go")
        time.sleep(1)
        if T32_R5A.get_run_state() == 2:
            print("  Breakpoint hit at end of Populate_MMIC_Stream")
        else:
            print("  Breakpoint NOT hit at end of Populate_MMIC_Stream")
    except Exception:
        print("  Error: Could not set breakpoint")
    ...
    if len(final_values) > 1 and final_values[1] == 0:
        ...
        check1 = False
    # Check RFMalFunctionFlag = FALSE (0)
    if len(final_values) > 3 and final_values[3] == 0:
        print(f"  RFMalFunctionFlag: {final_values[3]} (Expected 0) - PASS")
        check2 = True
    else:
        val = final_values[3] if len(final_values) > 3 else 0
        print(f"  RFMalFunctionFlag: {val} (Expected 0) - FAIL")
        check2 = False
