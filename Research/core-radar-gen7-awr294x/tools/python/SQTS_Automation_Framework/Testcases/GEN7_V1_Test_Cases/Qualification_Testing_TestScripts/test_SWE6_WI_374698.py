...
@pytest.mark.WI("WI-374698")
@pytest.mark.description("Qualification Test for Positive Validation of MMIC Low Temperature Fault Clearing")
def test_FRONT_END_MANAGEMENT(Power, T32_R5A, T32_C66, Report):
    ...
    if final_condition == 1:
        ...
        # Check: XCP_MMIC_FIT_Enable == 0, MMIC_Low_Temperature_Fault == 85, platform_fault == 0
        if values[0] == 0 and values[1] == 85 and values[2] == 0:
            final_condition = 1
        else:
            print("#MMIC_Low_Temperature_Fault is not set correctly in free run#")
            final_condition = 0
        print("")
        print("-----------------------------------WI-374698 :END-------------------------------------")
    else:
