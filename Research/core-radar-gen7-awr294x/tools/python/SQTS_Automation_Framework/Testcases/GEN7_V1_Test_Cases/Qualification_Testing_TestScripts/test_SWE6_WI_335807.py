...
@pytest.mark.WI("WI-335807")
@pytest.mark.description("Test to check RadarDiagShutdown UpdateFlag")
def test_FAULT_MANAGER(Power, T32_R5A, T32_C66, Report):
    ...
    if T == 3:
        ...
        flag = False
    temp = "RadarDiagRequest.RadarDiagShutdown"
    T32_R5A.add_var_watch(temp)
    TEMP_1 = T32_R5A.read_var(temp)
    print("Value of RadarDiagRequest.RadarDiagShutdown in free run is ", TEMP_1)
    temp_1 = "Platform_Active_Fault_Table"
    T32_R5A.add_var_watch(temp_1)
    temp_2 = "Platform_Active_Fault_Table.platform_bits.battery_low_fault"
    ...
    time.sleep(5)
    T = T32_R5A.read_var(t)
    print("Radar Status after changing the voltage to 6.9V is ", T_1[T])
    TEMP_2 = T32_R5A.read_var(temp_2)
    if TEMP_2 == 1:
        print("battery low fault is set when voltage is changed to 6.9V")
    else :
        print("battery low fault is not set when voltage is changed to 6.9v")

    TEMP_1 = T32_R5A.read_var(temp)
    print("Value of RadarDiagRequest.RadarDiagShutdown when battery_low_fault is set ", TEMP_1)
    ...
    print("-----------------------------------WI-335807 :END-------------------------------------")
    condition = 0
    if TEMP_1 == 170 and TEMP_2 == 1 and flag:
        print("Passed")
        condition = 1
    else :
        condition = 0
