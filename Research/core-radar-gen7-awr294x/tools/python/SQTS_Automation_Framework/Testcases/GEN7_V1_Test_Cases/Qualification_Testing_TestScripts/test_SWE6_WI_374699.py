...
@pytest.mark.WI("WI-374699")
@pytest.mark.description("Qualification Test for Negative Validation of MMIC Low Temperature Fault Clearing")
def test_FRONT_END_MANAGEMENT(Power, T32_R5A, T32_C66, Report):
    ...
    if final_condition == 1:
        ...
        print("##Overwriting XCP_MMIC_FIT_Enable to XCP_MMIC_Low_Temp_FIT")
        T32_R5A.cmd("Var.set %e XCP_MMIC_FIT_Enable = 3")
        time.sleep(3)
        ...
        low_temp_fault = T32_R5A.read_var(temp[1])

        print(f"{temp[0]} = {xcp_fit_enable}")
        print(f"{temp[1]} = {low_temp_fault}")
        print("")

        if low_temp_fault == 170:
            final_condition = 1
        else:
            print("#MMIC_Low_Temperature_Fault is not set after changing XCP_MMIC_FIT_Enable to XCP_MMIC_Low_Temp_FIT#")
            final_condition = 0
        print("")
        print("-----------------------------------WI-374699 :END-------------------------------------")
    else:
