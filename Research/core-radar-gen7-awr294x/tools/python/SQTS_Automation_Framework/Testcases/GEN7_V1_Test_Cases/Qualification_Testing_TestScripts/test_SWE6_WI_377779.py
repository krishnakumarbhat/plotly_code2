...
@pytest.mark.WI("WI-377779")
@pytest.mark.description("Qualification test for Negative validation of Idle to Normal mode(Over Temperature Handling)")
def test_FRONT_END_MANAGEMENT(Power, T32_R5A, T32_C66, Report):
    ...
    if final_condition == 1 :
        ...
        print("Overwritting XCP_MMIC_FIT_Enable to XCP_MMIC_OT_Idle_FIT")
        T32_R5A.cmd("Var.set %e XCP_MMIC_FIT_Enable = 6")
        time.sleep(1)
        for i in range(0, len(temp)) :
            time.sleep(2)
        ...
        if TEMP[1] != 0 and TEMP[2] > 0 and TEMP[3] == 8 :
            print("After changing XCP_MMIC_FIT_Enable to XCP_MMIC_OT_Idle_FIT, XCP_Over_Temperature_Api_Counter is incrementing")
            degraded_condition = 1
        else :
            print("After changing XCP_MMIC_FIT_Enable to XCP_MMIC_OT_Idle_FIT, XCP_Over_Temperature_Api_Counter is not incrementing")
        ...
        while TEMP[2] != 0 :
            ...
            if (attempts > 8) :
                break
        print("")
        print("##After XCP_Over_Temperature_Api_Counter reached the threshold limit 900##")
        for i in range(0, len(temp)) :
            time.sleep(0.5)
            TEMP[i] = T32_R5A.read_var(temp[i])
            if i == 3 :
        ...
        if TEMP[3] == (4 or 5) :
            ...
            normal_condition = 0
        final_condition = degraded_condition and idle_condition and normal_condition
        print("")
        print("-----------------------------------WI-377779 :END-------------------------------------")
    else :
