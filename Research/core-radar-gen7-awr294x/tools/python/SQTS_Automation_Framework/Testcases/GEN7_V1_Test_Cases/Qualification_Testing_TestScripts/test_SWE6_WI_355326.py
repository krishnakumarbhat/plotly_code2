...
@pytest.mark.WI("WI_355326")
@pytest.mark.description("Verify the IPC crc fault observed on DSS core is sent to MSS core")
def test_MCU_Configuration_And_Supervision(Power, T32_R5A, T32_C66, Report):
    ...
    variables = [
        "Ipc_D2m_Crc_Err_Count",
        "ipc_mss_dsp_out_msg.payload.errorField",
        "Ipc_M2d_Crc_Err_Count",
        "ipc_mss_dsp_in_msg.payload.errorField",
        "ipc_mss_sp_post_proc_read_msg.payload.errorField"
    ]
    values = []
    ...
    temp_2, temp_6, temp_3, temp_4, temp_5 = values[0], values[1], values[2], values[3], values[4]
    time.sleep(1)
    print("-----------------------------------WI-355326 :END-----------------------------------")
    condition = temp_2 == 0 and temp_3 == 0 and temp_4 == 0 and temp_5 == 0 and temp_6 == 0 and flag
    if condition:
        print("Passed")
        assert True
    else:
