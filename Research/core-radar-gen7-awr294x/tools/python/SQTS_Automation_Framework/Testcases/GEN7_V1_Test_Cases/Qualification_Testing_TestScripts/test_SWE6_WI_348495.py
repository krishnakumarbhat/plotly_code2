...
@pytest.mark.WI("WI_348495")
@pytest.mark.description("Qualification test for MMIC Data Discard - Positive Test Case")
def test_FRONT_END_MANAGEMENT(Power, T32_R5A, T32_C66, Report):
    ...
    t1 = "Radar_Data_Status.radar_data_status_payload.Radar_Data_Discard"
    temp_2 = T32_R5A.read_var(t1)
    print(t1, "=", temp_2)
    time.sleep(1)
    print("")
    print("-----------------------------------WI-348495 :END-------------------------------------")
    if temp_2 == 85 and flag:
        print("Passed")
        assert True
    else:
        print("In free run Radar_Data_Status.Payload.Radar_Data_Discard is ", temp_2)
        print("failed")
