...
@pytest.mark.WI("WI-375782")
@pytest.mark.description("Qualification Test for ECO_Mode_flag is enabled - Negative Case")
def test_FRONT_END_MANAGEMENT(Power, T32_R5A, T32_C66, Report):
    ...
    if final_condition == 1:
        print("-----------------------------------WI-375782 :START-----------------------------------")
        print("")
        temp = ["Eco_Mode_Status", "(Tx_Gain_Temp_Lut_Store[0][0].txGainTempLut)[0]", "(Tx_Gain_Temp_Lut_Store[0][1].txGainTempLut)[0]",
        ...
        # Add variable watches and read values
        for var in temp:
            T32_R5A.add_var_watch(var)

        time.sleep(0.5)
        eco_mode_status = T32_R5A.read_var(temp[0])
        tx_gain_0 = T32_R5A.read_var(temp[1])
        tx_gain_1 = T32_R5A.read_var(temp[2])
        tx_gain_2 = T32_R5A.read_var(temp[3])
        tx_gain_3 = T32_R5A.read_var(temp[4])
        tx_gain_4 = T32_R5A.read_var(temp[5])

        print(f"{temp[0]} = {eco_mode_status}")
        print(f"{temp[1]} = {tx_gain_0}")
        print(f"{temp[2]} = {tx_gain_1}")
        print(f"{temp[3]} = {tx_gain_2}")
        print(f"{temp[4]} = {tx_gain_3}")
        print(f"{temp[5]} = {tx_gain_4}")

        if eco_mode_status == 0x55 and tx_gain_0 != 0 and tx_gain_1 != 0 and tx_gain_2 != 0 and tx_gain_3 != 0 and tx_gain_4 != 0:
            final_condition = 1
        else:
            final_condition = 0
        print("")
