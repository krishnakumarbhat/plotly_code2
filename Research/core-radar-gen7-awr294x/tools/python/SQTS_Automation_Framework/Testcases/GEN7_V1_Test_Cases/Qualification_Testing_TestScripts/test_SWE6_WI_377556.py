...
@pytest.mark.WI("WI-377556")
@pytest.mark.description("Qualification Test for loopback - Negative Test Case")
def test_FRONT_END_MANAGEMENT(Power, T32_R5A, T32_C66, Report):
    ...
    if final_condition == 1 :
        print("-----------------------------------WI-377556 :START-----------------------------------")
        print("")
        temp = ["XCP_MMIC_FIT_Enable", "MMIC_Reconfig", "MMIC_Stream.MMIC_Data.MMIC_Fault_Status.MMIC_Loop_Back_Fault.loop_back_test_integrity_error",
                "MMIC_Error.Discard_Error_Count", "Platform_Active_Fault_Table.platform_bits.mmic_loop_back_test_fault"]
        TEMP = [0, 0, 0, 0, 0]
        for i in range(0, len(temp)) :
            T32_R5A.add_var_watch(temp[i])
        print("")
        print("##After changing XCP_MMIC_FIT_Enable to XCP_MMIC_Loop_back_FIT##")
        print("")
        T32_R5A.cmd("Var.set %e XCP_MMIC_FIT_Enable = 15")
        time.sleep(1)
        ...
        for i in range(0, len(temp)) :
            ...
            print(temp[i], "=", TEMP[i])
        if TEMP[2] == 170 and TEMP[3] > 3 and TEMP[4] == 1 :
            final_condition = 1
        else :
            final_condition = 0
        print("")
