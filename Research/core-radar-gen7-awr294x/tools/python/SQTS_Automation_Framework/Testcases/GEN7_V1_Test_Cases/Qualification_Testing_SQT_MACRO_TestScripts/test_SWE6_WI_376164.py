...
@pytest.mark.WI("WI_376164")
@pytest.mark.description("Write of TCMB application by Asil application")
def test_MCU_Configuration_And_Supervision(Power, T32_R5A, T32_C66, Report):
    ...
    if condition == 1:
        ...
        print(temp, "=", Temp)
        time.sleep(0.5)
        T32_R5A.cmd("Var.set %e MpuTestCase = 13")
        print("MpuTestCase to MPU_TCMB_WRITE_ACCESS_ASIL")
        if T == 3 :
            print(t, " ", T_1[T])
            condition_1 = 1
