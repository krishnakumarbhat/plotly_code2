...
@pytest.mark.WI("WI-370338")
@pytest.mark.description("External Watchdog Fault")
def test_FAULT_MANAGER(Power, T32_R5A, T32_C66, Report):
    ...
    if condition == 1:
        temp = "test_flag_externalwdg_fault"
        T32_R5A.add_var_watch(temp)
        Temp = T32_R5A.read_var(temp)
        print(temp, "=", Temp)
        temp_1 = "Platform_Active_Fault_Table.platform_bits.external_wdg_fault"
        T32_R5A.add_var_watch(temp_1)

        time.sleep(0.5)
        T32_R5A.cmd("Var.set %e test_flag_externalwdg_fault = 1")
        time.sleep(2)
        ...
        if Temp_1 == 1:
            ...
            condition_1 = 0
            print("Platform_Active_Fault_Table.platform_bits.external_wdg_fault is Not Set")
        time.sleep(0.5)
        T32_R5A.cmd("Var.set %e test_flag_externalwdg_fault = 0")
        time.sleep(2)
        Temp_1 = T32_R5A.read_var(temp_1)
        if Temp_1 == 0:
            condition_2 = 1
            ...
            condition_2 = 0
            print("Platform_Active_Fault_Table.platform_bits.external_wdg_fault is Not Cleared")
