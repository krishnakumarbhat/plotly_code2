...
if __name__ == "__main__":
    ...
    if os.path.exists(report_path):
        ...
        metric_name = f"core_radar.gen7_v1.{test_type}.testcases"

        # Send overall test results
        platformHealthInst.send_metric(
            metric_name,
            passed_tests,
            tags={"Type": "passed", "Variant": variant, "GenVersion": "Gen7v1"},
        )
        platformHealthInst.send_metric(
            metric_name,
            failed_tests,
            tags={"Type": "failed", "Variant": variant, "GenVersion": "Gen7v1"},
        )
        platformHealthInst.send_metric(
            metric_name,
            skipped_tests,
            tags={"Type": "skipped", "Variant": variant, "GenVersion": "Gen7v1"},
        )
        platformHealthInst.send_metric(
            metric_name,
            errors_tests,
            tags={"Type": "errors", "Variant": variant, "GenVersion": "Gen7v1"},
        )
        ...
        print(
            f"\n--- Overall/Modulewise Data sent to Platform Health for Gen7v1 Commit: {commit_id} ---\n"
        )
    else:
