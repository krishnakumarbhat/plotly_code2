...
BOOTLOADER_UNIFLASH_BUF_SIZE                         = 2097152 # This has to match BOOTLOADER_UNIFLASH_MAX_FILE_SIZE in boot_main.c
BOOTLOADER_UNIFLASH_HEADER_SIZE                      = 32 # 32 B
BOOTLOADER_UNIFLASH_BUF_SIZE_FOR_PART                = 1441792 # this is the size for the each part file used for send_file_by_parts
...
def xmodem_send_receive_file(filename, serialport, baudrate=115200, get_response=True):
    ...
    try:
        ...
        tstart = time.time()
        status = modem.send(stream, quiet=True, timeout=10, retry=50)
        tstop = time.time()
...
def send_file_by_parts(l_cfg, s_port):
    ...
    num_parts   = int(len(f_bytes) / BOOTLOADER_UNIFLASH_BUF_SIZE_FOR_PART)
    remain_size = len(f_bytes) % BOOTLOADER_UNIFLASH_BUF_SIZE_FOR_PART
    total_time_taken = 0


    for i in range(0, num_parts):

        start = i*BOOTLOADER_UNIFLASH_BUF_SIZE_FOR_PART
        end = start+BOOTLOADER_UNIFLASH_BUF_SIZE_FOR_PART
        ...
        l_cfg.offset = hex(get_numword(orig_offset) + i*BOOTLOADER_UNIFLASH_BUF_SIZE_FOR_PART)
    ...
    if(remain_size > 0):
        start = num_parts*BOOTLOADER_UNIFLASH_BUF_SIZE_FOR_PART
        end = start+remain_size # Read till the end of original file
        ...
        # temporarily change this to the partial filename
        l_cfg.filename = part_filename
        l_cfg.offset = hex(get_numword(orig_offset) + num_parts*BOOTLOADER_UNIFLASH_BUF_SIZE_FOR_PART)
