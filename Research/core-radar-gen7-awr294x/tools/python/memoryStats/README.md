...
1. Every time a developer adds a new file to the project, they must map it to the appropriate module in the `\tools\python\memoryStats\module_filename_map.json` file. Otherwise, below error will appear during the build, and the file will be categorized under the Uncategorized section in the memory stats output:

    ```
    ERROR : Files listed below are not grouped into modules for memory scripts. Group these files in \tools\python\memoryStats\module_filename_map.json.
    ```
