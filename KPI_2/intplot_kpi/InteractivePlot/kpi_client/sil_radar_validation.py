...
def _scan_index_candidates(sensor: str) -> tuple:
    """Build scan_index candidate paths covering every known header-group spelling.

    Legacy spellings stay first so existing file formats are unaffected; the
    header-variant expansion handles files that use e.g. 'Stream_HDR' instead
    of 'Stream_Hdr' as the stream header group name.
    """
    candidates = [
    ...
    for stream in ("DETECTION_STREAM", "RDD_STREAM"):
        for variant in HEADER_VARIANTS:
            candidate = f"{sensor}/{stream}/{variant}/scan_index"
...
def _build_from_af_det_with_rdd_fallback(handle: h5py.File, sensor: str) -> Optional[LoadedRadar]:
    ...
    scan_candidates = _scan_index_candidates(sensor)
