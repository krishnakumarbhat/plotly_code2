...
class RagEngine:
    ...
    def add_sections(
        ...
        for chunk_index, section in enumerate(sections, start=1):
            ...
            if self._collection is not None:
                ...
                collection_embeddings.append(embedding)
    ...
    def _embed_text(self, text: str) -> list[float]:
        ...
        for i, token in enumerate(tokens):
            digest = hashlib.sha256(f"{i}:{token}".encode('utf-8', errors='ignore')).digest()
            for j in range(4):
                idx = (int.from_bytes(digest[j*4:(j+1)*4], 'big') % (self._embedding_dimension - j)) + j
                sign = 1.0 if digest[j] % 2 == 0 else -1.0
                weight = (1.0 + min(len(token), 32) / 32.0) * (1.0 / (1.0 + j * 0.5))
                vector[idx] += sign * weight

        norm = math.sqrt(sum(c * c for c in vector)) or 1.0
