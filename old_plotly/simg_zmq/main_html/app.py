...
from utils import SlurmManager, FileBrowser, extract_hdf_filename, slurm_is_available, cluster_from_path, validate_cluster_credentials
...
def _set_hyperlink_cache_headers(response):
    ...
    elif path.startswith('/hyperlink/data/html/') or path.startswith('/hyperlink/static/') or path.startswith('/static/'):
        response.headers.setdefault('Cache-Control', f'public, max-age={app.config.get("STATIC_CACHE_MAX_AGE", 31536000)}')
...
def submit_tool_job(tool_name: str):
    ...
    singularity_images = {
        ...
        'kpi': 'kpi.simg',
        'hyperlink_tool': 'main_html.simg'
    }
    
    singularity_image = os.path.join(
        simg_folder,
        singularity_images.get(tool_name, 'main_html.simg')
    )
