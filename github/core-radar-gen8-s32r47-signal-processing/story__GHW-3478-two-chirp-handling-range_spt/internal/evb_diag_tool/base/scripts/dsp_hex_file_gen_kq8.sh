...
#
# @file dsp_hex_file_gen_kq8.sh
# @brief Converts KQ8 elf file to C data arrays.
...
if [ -z "${ELF_NAME}" ] || [ -z "${STB_DIR}" ] || [ -z "${OUT_NAME}" ] || [ -z "${PLATFORM}" ]; then
    ...
    echo "Usage: dsp_hex_file_gen_kq8.sh ELF_NAME STB_DIR OUT_NAME PLATFORM [OUT_DIR] [CORE] [INST_ID]"
    exit 1
fi
...
if [ $PLATFORM == "S32R45" ]; then
    ...
    if [ $CORE == "BBE32" ]; then
        ...
        if [ $INST_ID == "0" ]; then
            DRAM0_BASE_ADDR=0x21000000
            DRAM1_BASE_ADDR=0x21040000
            IRAM0_BASE_ADDR=0x21080000
        elif [ $INST_ID == "1" ]; then
            DRAM0_BASE_ADDR=0x21000000
            DRAM1_BASE_ADDR=0x21040000
            IRAM0_BASE_ADDR=0x21080000
...
#        echo "uint8_t dsp_img_segment_${curentLine:1:10}[] __attribute__((section("\".dsp_img_segment_${curentLine:1:10}\""))) = {"
      segAddresses="${segAddresses}${curentLine:1:10}, "
        if [ $INST_ID == "0" ]; then
          echo "uint8_t kq8_0_img_segment_$idx[] = {"
        elif [ $INST_ID == "1" ]; then
          echo "uint8_t kq8_1_img_segment_$idx[] = {"
        fi
      ((idx++));
    else
  ...
  if [ $INST_ID == "0" ]; then
    echo "uint8_t gNumKq80ImgSegments = $idx;"
  elif [ $INST_ID == "1" ]; then
    echo "uint8_t gNumKq81ImgSegments = $idx;"
  fi
  echo ''

  #echo "uintptr_t dspImgSegmentRunaddr[RSDK_NUM_DSP_IMG_SEGMENTS] = {$segAddresses};"
  if [ $INST_ID == "0" ]; then
  echo "uintptr_t kq80ImgSegmentRunaddr[] = {$segAddresses};"
  elif [ $INST_ID == "1" ]; then
  echo "uintptr_t kq81ImgSegmentRunaddr[] = {$segAddresses};"
  fi
  echo ''
  ...
  if [ $INST_ID == "0" ]; then
  echo "uint32_t kq80ImgSegmentSizes[] = {"
  elif [ $INST_ID == "1" ]; then
  echo "uint32_t kq81ImgSegmentSizes[] = {"
  fi
  for i in $(seq 0 $idx); do
  if [ $INST_ID == "0" ]; then
  echo "    sizeof(kq8_0_img_segment_$i)",
  elif [ $INST_ID == "1" ]; then
  echo "    sizeof(kq8_1_img_segment_$i)",
  fi
  done
  echo "};"
  echo ''
  ...
  if [ $INST_ID == "0" ]; then
  echo "uint8_t* kq80ImgSegmentLoadAddr[] = {"
  elif [ $INST_ID == "1" ]; then
  echo "uint8_t* kq81ImgSegmentLoadAddr[] = {"
  fi
  for i in $(seq 0 $idx); do
  if [ $INST_ID == "0" ]; then
  echo "    kq8_0_img_segment_$i",
  elif [ $INST_ID == "1" ]; then
  echo "    kq8_1_img_segment_$i",
  fi
  done
  echo "};"
  echo ''
...
# APTIV REMOVED
# cp ${OUTPUT_FILE_NAME} ../../Diag_Tool_M7/src/radar/kq8/src
# echo
# echo copied to: ../../Diag_Tool_M7/src/radar/kq8/src/${OUT_NAME}
