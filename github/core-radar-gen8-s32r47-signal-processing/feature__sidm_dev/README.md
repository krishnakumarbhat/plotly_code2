...
SIDM under doppler SPBB:
Note: This kernel is first cut for SIDM and is only running on 8xRX of data. This will further need to be modified for integration into the doppler flow.
(1)User will need to run the create_test_data_sidm.m file with some .mat file that has the rfft and restored rfft results from the agnostic specs, along with some configs needed for SIDM such as thresholding constants.
(2)Then, run the file called doppler_sidm_spt_sim.m. This will load the output of previous step into the relevant smem or oram memory addresses for the kernel to pdma it in.
(3)At the end of successful kernel execution the outputs from smem will be read back and compared against the restored rfft data from the .mat file that was used in step(1)
(4)Embedded verification for this SPBB on EVB will come in the coming reviews.
