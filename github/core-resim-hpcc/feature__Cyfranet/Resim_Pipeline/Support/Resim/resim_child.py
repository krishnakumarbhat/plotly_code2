...
elif [[ $SIMG_NAME == *'resim_gen8'* ]]; then
    YIELD_CUST="GEN8"
...
elif [[ $SIMG_LOWER == *'resim_gen8'* ]]; then
    YIELD_CUST="GEN8"
...
mkdir -p $TMPOUT 
# TASK_LINE (not SLURM_ARRAY_TASK_ID) - Helios chunking restarts SLURM_ARRAY_TASK_ID
# at 1 for every chunk, so two different chunks' tasks would otherwise collide on the
# same JSON_FILE name and overwrite/read each other's session data mid-run.
JSON_FILE="$JOBOUT/SIL_input_${TASK_LINE}.json"
