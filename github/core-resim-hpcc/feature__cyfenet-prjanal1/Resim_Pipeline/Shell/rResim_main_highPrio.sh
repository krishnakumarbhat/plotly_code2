...
# This script is shared by Helios and Krakow (routed via trig_pip.sh). Only Helios's
# cluster-configured SLURM MaxArraySize rejects array jobs beyond ~1000 tasks with
# "Invalid job array specification", so only Helios (EXECUTION_PROJECT=="8k3p89",
# set in resim_generic.py for 8k3* usernames) needs chunked submission; Krakow
# handles arbitrarily large arrays fine as a single job.
if [[ $EXECUTION_PROJECT == "8k3p89" ]]; then
    MAX_TASKS=10000
    JOBS=$(( (${TASKS} - 1) / ${MAX_TASKS} ))
    ...
    for (( i = 0; i <= JOBS; i++ )); do
        ...
        CHUNK_JOB_ID=$(sbatch -A $EXECUTION_PROJECT $PARTITION -a "1-$TASK_LENGTH%$TASK_LENGTH" $CHILD $TASK_START $SIMG $OUT $UPU $CONFIG | awk '{print $NF}')
        if [[ -n "$CHUNK_JOB_ID" ]]; then
            JOB_ID="${JOB_ID:+$JOB_ID,}$CHUNK_JOB_ID"
        fi
    done
else
    ...
    # TASK_START offset of 0 for resim_child.py's SLURM_ARRAY_TASK_ID + TASK_START line-number lookup.
    JOB_ID=$(sbatch -A $EXECUTION_PROJECT $PARTITION -a "1-$TASKS%$TASKS" $CHILD 0 $SIMG $OUT $UPU $CONFIG | awk '{print $NF}')
fi

# JOB_ID may be comma-joined (Helios multi-chunk) - the --dependency flag needs a
# colon-separated list, hence the separate variable (tr is a no-op for a single id).
JOB_ID_DEP=$(echo "$JOB_ID" | tr ',' ':')

JOB_ID1=$(sbatch -A $EXECUTION_PROJECT --dependency=afterany:$JOB_ID_DEP -p $MINING_PARTITION $RMIN $OUTPUT/.mining.txt $CUST $CUST | awk '{print $NF}')
JOB_ID2=$(sbatch -A $EXECUTION_PROJECT --dependency=afterany:$JOB_ID_DEP -p $MINING_PARTITION $SMIN $OUTPUT/.SIL_Statistics.txt | awk '{print $NF}')
