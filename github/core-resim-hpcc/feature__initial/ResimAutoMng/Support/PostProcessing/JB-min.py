...
#
##############################################################
def data_mining(raw,inputFiles,cnt):
    global slownessAvgJR,slownessAvgRR,slownessAvgHR,slownessAvgBR,slownessAvgMiR,slownessAvgMoR
    ...
    for message in raw:
        if fw_version in message: resimComp="Running"
            ...
            if resimComp == "Running":
                ...
                if logsRan>=1:
                    logsRan-=1
            elif htmlComp == "Running": htmlComp = 'Crash'
            elif bordIComp == "Running": bordIComp = 'Crash'
            elif bordOComp == "Running": bordOComp = 'Crash'
            elif mudpIComp == "Running": mudpIComp = 'Crash'
            elif mudpOComp == "Running": mudpOComp = 'Crash'
            else: pass
            inputFiles[1].write(f'{currentInput}\n')
            ...
            jobComp=True
            if timeoutAlert == False and oomAlert == False and calibPAlert == False and calibFAlert == False and mdfAlert == False:
                if resimComp!="Done" or htmlComp!="Done" or bordIComp!="Done" or bordOComp!="Done" or mudpIComp!="Done" or mudpOComp!="Done":
                    jobStatus = "Partial"
                else:
                    jobStatus = "Done"
    ...
    #if jobComp == True:
    if jobTime != 0:
            try:jobRate=round(float(jobTime)/(sessionTime),2)
            except:print("jobRate Exception :",jobTime, logsRan, sessionTime)
    if resimTime != 0:
            try:resimRate=round(float(resimTime)/(sessionTime),2)
            except:print("resimRate Exception :",resimTime, logsRan, sessionTime)
    if htmlTime != 0:
            try:htmlRate=round(float(htmlTime)/(sessionTime),2)
            except:print("htmlRate Exception :",htmlTime, logsRan, sessionTime)
    if bordITime != 0:
            try:bordIRate=round(float(bordITime)/(sessionTime),2)
            except:print("bordIRate Exception :",bordITime, logsRan, sessionTime)
    if bordOTime != 0:
            try:bordORate=round(float(bordITime)/(sessionTime),2)
            except:print("bordORate Exception :",bordITime, logsRan, sessionTime)

    if mudpITime != 0:
            try:mudpIRate=round(float(mudpITime)/(sessionTime),2)
            except:print("mudp input Rate Exception :",mudpITime, logsRan, sessionTime)
    if mudpOTime != 0:
            try:mudpORate=round(float(mudpOTime)/(sessionTime),2)
            except:print("mudp Output Rate Exception :",mudpOTime, logsRan, sessionTime)
    ...
    inputFiles[0].write(f" {logversion}".ljust(14, ' ')+f"| {session}".ljust(9, ' ')+f"| {jobStatus}".ljust(10, ' ')+f"| {sessionYield}".ljust(9, ' ')+f"| {entrymode}".ljust(12, ' ')+f"| {fileexecution}".ljust(12, ' ')+f"| {blc}".ljust(14, ' ')+f"| {totalLogs}".ljust(9, ' ')+f"| {sessionTime}".ljust(9, ' ')+f"| {logsRan}".ljust(9, ' ')+f"| {logNotRan}".ljust(9, ' ')+f"| {badLogs}".ljust(9, ' ')+f"| {logCrashed}".ljust(9, ' ')+f"| {resimComp}".ljust(8,' ')+f"| {htmlComp}".ljust(8,' ')+f"| {bordIComp}".ljust(8,' ')+f"| {bordOComp}".ljust(8,' ')+f"| {mudpIComp}".ljust(8,' ')+f"| {mudpOComp}".ljust(8,' ')+f"| {busSpecid}".ljust(9,' ')+f"| {jobTime}".ljust(9,' ')+f"| {resimTime}".ljust(9,' ')+f"| {htmlTime}".ljust(9,' ')+f"| {bordITime}".ljust(9,' ')+f"| {bordOTime}".ljust(9,' ')+f"| {mudpITime}".ljust(9,' ')+f"| {mudpITime}".ljust(9,' ')+f"| {jobRate}".ljust(9,' ')+f"| {resimRate}".ljust(9,' ')+f"| {htmlRate}".ljust(9,' ')+f"| {bordIRate}".ljust(9,' ')+f"| {bordORate}".ljust(9,' ')+f"| {mudpIRate}".ljust(9,' ')+f"| {mudpIRate}".ljust(9,' ')+f"| {currentInput}".ljust(210,' ')+f"| {badLogList}".ljust(70,' ')+'\n')
    ...
    if jobRate != -1 :slownessAvgJR.append(jobRate)
    if resimRate != -1 :slownessAvgRR.append(resimRate)
    if htmlRate != -1 :slownessAvgHR.append(htmlRate)
    if bordIRate != -1 :slownessAvgBiR.append(bordIRate)
    if bordORate != -1 :slownessAvgBoR.append(bordORate)
    if mudpIRate != -1 :slownessAvgMiR.append(mudpIRate)
    if mudpORate != -1 :slownessAvgMoR.append(mudpORate)

    '''print(slownessAvgJR)
    print(slownessAvgRR)
    print(slownessAvgHR)
    print(slownessAvgBiR)
    print(slownessAvgBoR)
    print(slownessAvgMiR)
    print(slownessAvgMoR)'''



# html mining data update
    row_data=[cnt,logversion,session,sessionYield,entrymode,fileexecution,blc,sessionTime,jobStatus,resimComp,htmlComp,bordIComp,bordOComp,mudpIComp,mudpOComp,totalLogs,logsRan,logNotRan,badLogs,logCrashed,jobTime,resimTime,htmlTime,bordITime,bordOTime,mudpITime,mudpOTime,jobRate,resimRate,htmlRate,bordIRate,bordORate,mudpIRate,mudpORate,badLogList,currentInput]
    html_mining_data.append(row_data)
    html_SlowReSim.append(resimRate)
    html_SlowHTML.append(htmlRate)
    html_SlowIBORD.append(bordIRate)
    html_SlowOBORD.append(bordORate)
    html_SlowJob.append(jobRate)
    html_SlowIMUDP.append(mudpIRate)
    html_SlowOMUDP.append(mudpORate)
    html_Slowverall.append([jobRate,resimRate,htmlRate,bordIRate,bordORate,mudpIRate,mudpORate])
...
# writing data to xlsx file
    content=["Logs_Version", 'Session', 'Job_Status', 'Session_Yield', 'SIL Mode', 'File Mode', 'Bad Log Check', 'Logs_Total', 'Logs_Duration', 'Logs_Executed', 'Logs_NotExecuted', 'Logs_Bag', 'Log_Crashed', 'ReSim_Status', 'HTML_Status', 'I-BORD_Status', 'O-BORD_Status', 'I-MUDP_Status', 'O-MUDP_Status', 'Missing_BusSpecId', 'TotalTime_Job', 'TotalTime_ReSim', 'TotalTime_HTML', 'TotalTime_I-BORD', 'TotalTime_O-BORD', 'TotalTime_I-MUDP', 'TotalTime_I-MUDP', 'SlownessRate_Job', 'SlownessRate_ReSim', 'SlownessRate_HTML', 'SlownessRate_I-BORD', 'SlownessRate_O-BORD', 'SlownessRate_I-MUDP', 'SlownessRate_O-MUDP', 'Session_Details', 'BadLogs_Encountered ']
    content_scan=[]
    content_scanLog=[]
    ...
    for detail in all_details:
        ...
        if line >= currLine and versionCaptured == True:
            ...
            content[20]=float(content[20])
            content[21]=float(content[21])
            content[22]=float(content[22])
            content[23]=float(content[23])
            content[24]=float(content[24])
            content[25]=float(content[25])
            content[26]=float(content[26])
            content[27]=float(content[27])
            content[28]=float(content[28])
            content[29]=float(content[29])
            content[30]=float(content[30])
            content[31]=float(content[31])
            content[32]=float(content[32])
            content[33]=float(content[33])
            ...
            if content[2] == ' Partial ' and content[12] !=  ' False  ':
                sessionCrashed+=1
                logsCrashed += content[10]-content[11]
            if content[2] == ' TimeOut ':
                sessionTimeout+=1
                logsTimeout+=(content[10]-content[11])
            if content[2] == ' OOM     ':
                sessionOOM+=1
                if content[12] ==  ' Running':
                    if content[11] == ' False  ': logsOOM+=(content[10]-content[11])
                    else: logsCrashed += content[10]-content[11]
                else:
                    sessionPCalib+=1
                    logsPCalib+=(content[10]-content[11])
            if content[2] == ' CALIB_P ' or content[2] == ' CALIB_F ' :
                if content[10]-content[11] == 0:
                ...
                elif (content[10]-content[11]) == content[10]:
                    content[2] = ' CALIB_F '
                    sessionFCalib+=1
                    logsFCalib+=(content[10]-content[11])
...
#creating Pie chart for Pass-fail yield
    logs_unknown=logsTotal-(logsResimulated+logsCrashed+logsBad+logsBusspecId+logsTimeout+logsOOM+logsPCalib+logsFCalib+logsMdf)
    yieldSheet.merge_range("E1:L1",'Logs Version', header_prop)
    yieldSheet.merge_range("E2:L2",logVersion, content_prop)
    yieldSheet.write_row(21,5,['NA','NA','NA','NA','NA','NA'], content_prop)

    yieldSheet.merge_range("F20:K20",'AVG-Slowness Factor', header_prop)
    yieldSheet.write_row(20,5,["Job","ReSim","HTML","BORD","MUDP-I","MUDP-O"], content_prop)
    if len(slownessAvgJR) > 0: yieldSheet.write_row(21,5,[round(sum(slownessAvgJR)/len(slownessAvgJR),2)], content_prop)
    if len(slownessAvgRR) > 0: yieldSheet.write_row(21,6,[round(sum(slownessAvgRR)/len(slownessAvgRR),2)], content_prop)
    if len(slownessAvgHR) > 0: yieldSheet.write_row(21,7,[round(sum(slownessAvgHR)/len(slownessAvgHR),2)], content_prop)
    if len(slownessAvgBiR) > 0: yieldSheet.write_row(21,8,[round(sum(slownessAvgBiR)/len(slownessAvgBiR),2)], content_prop)
    if len(slownessAvgBoR) > 0: yieldSheet.write_row(21,8,[round(sum(slownessAvgBoR)/len(slownessAvgBoR),2)], content_prop)
    if len(slownessAvgMiR) > 0: yieldSheet.write_row(21,9,[round(sum(slownessAvgMiR)/len(slownessAvgMiR),2)], content_prop)
    if len(slownessAvgMoR) > 0: yieldSheet.write_row(21,10,[round(sum(slownessAvgMoR)/len(slownessAvgMoR),2)], content_prop)
    ...
    yieldSheet.write_row(10,0,['MDF Impact', round(logsMdf*100/logsTotal,2),logsMdf], content_prop)
    yieldSheet.write_row(11,0,['Unknown Impact', round(logs_unknown*100/logsTotal,2),logs_unknown], content_prop)
    yieldSheet.write_row(12,0,['Not_Resimulated_Overall', round(logsNotResimulated*100/logsTotal,2),logsNotResimulated], content_prop)
    ...
    yieldSheet.write_row(21,0,['MDF error',sessionMdf ], content_prop)

    chart1 = workbook.add_chart({'type': 'pie'})
    chart1.set_title({'name': 'ReSim_Execution_Yield (%)'})
    chart1.add_series({ 
    'name':       'Overall_JoboutMinning_Summary', 
    'categories': ['Yield', 2, 0, 11, 0],
    'values':     ['Yield', 2, 1, 11, 1],
    'points': [ {'fill': {'color': '#5cd390'}}, {'fill': {'color': '#ffa191'}}, {'fill': {'color': '#ffd700'}}, {'fill': {'color': '#f0e4d7'}}, {'fill': {'color': '#b6a9d6'}}, {'fill': {'color': '#cc2d39'}}, {'fill': {'color': '#0099ff'}}, {'fill': {'color': '#0066cc'}}, {'fill': {'color': '#000000'}}, {'fill': {'color': '#aaaaaa'}}],
...
#creating Histogram chart for ReSim Slowness
    ...
    'values'    : ['Mining_details',1,28,len(all_details),28],
    'line'	: {'color':'red'}
    })
...
#creating Histogram chart for HTML Slowness
    chart3 = workbook.add_chart({'type':'column'})
    chart3.set_x_axis({"name": "Session"})
    chart3.set_y_axis({"name": "Slowness"})
    chart3.add_series({
    'name'      : "HTML Slowness/Session",
    'categories': ['Mining_details',1,1,len(all_details),1],
    'values'    : ['Mining_details',1,29,len(all_details),29],
    'line'	: {'color':'red'}
    })

    chart3.set_style(12)
    plotSheet.insert_chart('B50', chart3, {'x_scale': 3, 'y_scale': 1})

#creating Histogram chart for BORDNET-INPUT Slowness
    chart3 = workbook.add_chart({'type':'column'})
    chart3.set_x_axis({"name": "Session"})
    chart3.set_y_axis({"name": "Slowness"})
    chart3.add_series({
    'name'      : "I-BORD Slowness/Session",
    'categories': ['Mining_details',1,1,len(all_details),1],
    'values'    : ['Mining_details',1,30,len(all_details),30],
    'line'	: {'color':'red'}
    })

    chart3.set_style(12)
    plotSheet.insert_chart('B66', chart3, {'x_scale': 3, 'y_scale': 1})

#creating Histogram chart for BORDNET_OUTPUT Slowness
    chart3 = workbook.add_chart({'type':'column'})
    chart3.set_x_axis({"name": "Session"})
    chart3.set_y_axis({"name": "Slowness"})
    chart3.add_series({
    'name'      : "O-BORD Slowness/Session",
    'categories': ['Mining_details',1,1,len(all_details),1],
    'values'    : ['Mining_details',1,31,len(all_details),31],
    'line'	: {'color':'red'}
    })

    chart3.set_style(12)
    plotSheet.insert_chart('B82', chart3, {'x_scale': 3, 'y_scale': 1})

#creating Histogram chart for MUDP_INPUT Slowness
    chart3 = workbook.add_chart({'type':'column'})
    chart3.set_x_axis({"name": "Session"})
    chart3.set_y_axis({"name": "Slowness"})
    chart3.add_series({
    'name'      : "I-MUDP Slowness/Session",
    'categories': ['Mining_details',1,1,len(all_details),1],
    'values'    : ['Mining_details',1,32,len(all_details),32],
    'line'	: {'color':'red'}
    })

    chart3.set_style(12)
    plotSheet.insert_chart('B98', chart3, {'x_scale': 3, 'y_scale': 1})

#creating Histogram chart for MUDP_OUTPUT Slowness
    chart3 = workbook.add_chart({'type':'column'})
    chart3.set_x_axis({"name": "Session"})
    chart3.set_y_axis({"name": "Slowness"})
    chart3.add_series({
    'name'      : "O-MUDP Slowness/Session",
    'categories': ['Mining_details',1,1,len(all_details),1],
    'values'    : ['Mining_details',1,33,len(all_details),33],
    'line'	: {'color':'red'}
    })

    chart3.set_style(12)
    plotSheet.insert_chart('B114', chart3, {'x_scale': 3, 'y_scale': 1})
...
if __name__ == '__main__':
    ...
    for job in jobouts:                                                                                         # mining on each file
        print(f"[MINING-INFO] : Jobout Mining {cnt}/{len(jobouts)} -> {job}")
        job_file=open(job.split('\n')[0],'r')
        job_content=job_file.readlines()
        job_file.close()
        
        data_mining(job_content,[tmp_file, tmp_abort_file, tmp_bad_file, tmp_bad_session_file, scan_bad_session,scanLog_bad_session, kpi_session, kpiLog_session], cnt)
        cnt+=1
