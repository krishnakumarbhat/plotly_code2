...
for (( i = 0; i <= JOBS; i++ )); do
    ...
    JOB_ID=$(sbatch -A RNA-SDV-SRR7 -a "1-$TASKS_LENGTH%$TASKS_LENGTH" $SCRIPT_VV $TASKS_START $RESIM_SIMG $OUT_PATH | awk '{print $NF}')
done

MIN_JOB_ID=$(sbatch -A RNA-SDV-SRR7 --dependency=afterany:$JOB_ID $VV_MINING $OUT_PATH/output/.mining.txt $JOBOUT | awk '{print $NF}')
