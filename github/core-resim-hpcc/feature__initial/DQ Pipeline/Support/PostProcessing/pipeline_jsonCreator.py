...
#
###################################################
def validate_outputFiles (output_path, simg):
    error = False
...
#
###################################################
def collect_output_files(out_path,toolAsked, simg):
...
def write_mudp(path,files, path_out, requestFor):
    ...
    if requestFor =="i": txtfile=f"{mudpPath}/input.txt"
    else: txtfile=f"{mudpPath}/output.txt"
...
def write_htmlJson(path_i, path_o, files_i, files_o, path_out):
    ...
    files_il.sort()
    files_ol.sort()
    file = f'{jsnoPath}/HTMLInputs.json'
...
#
###################################################
def write_bordJson(files_i, files_o, path_out ):
    jsnoPath = f'{path_out}/BORDNET_REPORT'
    try:os.mkdir(jsnoPath)
    except: pass
    file = f'{jsnoPath}/BORDNET_fList.json'
    jsonFile = open(file, 'w')
    jsonFile.write('{\n    "reprocessingInputFileStreams": [\n')
    updateJson(jsonFile, None, [], "INPUT_BN_CALIFR")
    if 'stla_scale1' in simg:
        updateJson(jsonFile, None, files_i[0], "INPUT_BN_FASETH")
        updateJson(jsonFile, None, files_i[0], "INPUT_SRR_DEBUG")
    elif 'stla_scale3' in simg:
        updateJson(jsonFile, None, files_i[1], "INPUT_BN_FASETH")
        updateJson(jsonFile, None, files_i[0], "INPUT_SRR_DEBUG")
    else:
        updateJson(jsonFile, None, [], "INPUT_BN_FASETH")
        updateJson(jsonFile, None, files_i[2], "INPUT_SRR_DEBUG")
    
    updateJson(jsonFile, None, [], "OUTPUT_BN_CALIFR")
    if 'stla_scale1' in simg:
        updateJson(jsonFile, None, files_o[0], "OUTPUT_BN_FASETH")
        updateJson(jsonFile, None, files_o[0], "OUTPUT_SRR_DEBUG")
    elif 'stla_scale3' in simg:
        updateJson(jsonFile, None, files_o[1], "OUTPUT_BN_FASETH")
        updateJson(jsonFile, None, files_o[0], "OUTPUT_SRR_DEBUG")
    else:
        updateJson(jsonFile, None, [], "OUTPUT_BN_FASETH")
        updateJson(jsonFile, None, files_o[2], "OUTPUT_SRR_DEBUG")
    
    updateJson(jsonFile, None, [], "SRR_DUMMY")
    jsonFile.write('\t\t]\n}')
    jsonFile.close()
...
#
###################################################
def create_json_html (json_file,output_path):
    inp_path, inp_files = collect_input_files(json_file,'html')
    oup_path, oup_files = collect_output_files(output_path,'html', simg)
...
#
###################################################
def create_json_bordnet (json_file,output_path, simg):
    files_i = collect_input_files(json_file,'bordnet')
    files_o = collect_output_files(output_path,'bordnet', simg)
    write_bordJson(files_i, files_o, output_path )
...
if __name__ == '__main__':
    ...
    simg = sys.argv[3]
    flag = validate_outputFiles(output_path, simg)
    print('[DOCKER] :',flag)

    if flag[0] == 1:
        if "dc" in simg:
            create_dc_json_html(json_file,output_path)
        else:
            create_json_html(json_file,output_path)
    if flag[1] == 1:
        create_json_bordnet(json_file,output_path, simg)
