...
if [[ $SIMG_PATH == *'projects'* ]]; then
    ...
    if [[ $SIMG_PATH == *'traton'* ]]; then
        EXECUTION_PROJECT="TRATON-SRR6T"

    elif [[ $SIMG_PATH == *'honda'* ]]; then
        EXECUTION_PROJECT="HONDA-SRR6P"
    else
...
for (( i = 0; i <= JOBS; i++ )); do
    ...
    JOB_ID=$(sbatch -A $EXECUTION_PROJECT -p highPrio -a "1-$ARRAY_LENGTH%$ARRAY_LENGTH" $SCRIPT $ARRAY_START $SIMG_PATH $OUTPUT_PATH | awk '{print $NF}')
