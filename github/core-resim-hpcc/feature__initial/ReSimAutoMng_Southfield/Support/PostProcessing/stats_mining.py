...
def stat_func(filelist):
    ...
    # Guard: if no sensor data was parsed (e.g. DGPS XMLs), write error-only report and return
    if not isinstance(df_dict['RL'][0], pd.DataFrame):
        print('[STATS_MINING] : No standard sensor statistics found in XML files (DGPS mode?). Writing error-only report.')
