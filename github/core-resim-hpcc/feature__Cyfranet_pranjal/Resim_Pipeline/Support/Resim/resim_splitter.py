...
def createJson(path, curr_cnt, sessions):
    ...
    if input_parameter[5] == 'dSpace':
        ...
        if 'resim_stla_small' in simg_name:
            ...
            mpad = []
            ref = []
        elif 'resim_gen8' in simg_name:
            # Gen8 docker (resim_gen8_*.simg): MF4 files tagged with both "gen8" and
            # "upu" feed SRR_DEBUG; BN_CALIFR/BN_FASETH reuse the same stream
            # (deb deb deb), there is no separate bus/ref stream.
            upuFlag = True
            all_files = getfiles(path, '')
            for file in all_files:
                filename = file.split('/')[-1].lower()
                if 'gen8' in filename and 'upu' in filename:
                    deb.append(file)
            if not deb:
                print(f"[ERROR] : Gen8 - no files matching 'gen8' and 'upu' found under {path}")
            cbus = list(deb)
            mpad = list(deb)
            ref = []
...
def createFileList(path, curr_cnt, sessions):
    ...
    files=[]
    simg_name = os.path.basename(input_parameter[2]).lower()
    # Gen8 logs are much larger than other docker types; cap batch size to 5 logs/job instead of 15.
    singleSession = 5 if 'resim_gen8' in simg_name else 15
    if input_parameter[5] == 'dSpace':
        ...
        if 'resim_stla_small' in simg_name:
            ...
            # Gen8 docker (resim_gen8_*.simg): input logs are MF4 files tagged
            # with both "gen8" and "upu" (e.g. "..._Gen8_..._UPU.mf4").
            upuFlag = True
            all_files = getfiles(path, '', 1)
            files = [f for f in all_files if 'gen8' in os.path.basename(f).lower() and 'upu' in os.path.basename(f).lower()]
            if not files:
                print(f"[ERROR] : Gen8 - no files matching 'gen8' and 'upu' found under {path}")
        elif 'mcip' in simg_name :
