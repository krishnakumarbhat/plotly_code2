...
def childCreation(input_parameter):
    ...
    script_dir = os.path.dirname(os.path.abspath(__file__))   # script_dir = .../Core_RESIM_HPCC/VV_Resim_Pipeline
    vv_pipeline_dir = script_dir
    script = os.path.join(vv_pipeline_dir, 'rResimVV_main.sh')
    mining_script = os.path.join(vv_pipeline_dir, 'PostProcessing', 'VV_mining.py')
    mining = f"python {mining_script}"

    input_parameter.append(script)
    input_parameter.append(mining)
    
    # Determine the base path from current working directory (where the job is submitted)
    hpcc = os.getcwd()
    if 'projects' in hpcc:
        server = "projects/"
    else:
