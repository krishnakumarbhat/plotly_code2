...
### 7.6 Runtime Stream Transmission Enable/Disable

Streams can be enabled or disabled at runtime using Lauterbach (TRACE32) by updating the variables below:

1. Select the streams to be enabled/disabled using **Stream_Num_Bitfield[STREAM_NUM_BITFIELD_WORDS]**.
  - Each bit represents a stream number: stream $N$ is selected by setting bit $(N \bmod 32)$ in word index $(N / 32)$.
    - Example: stream 10 → **Stream_Num_Bitfield[0]** bit 10
    - Example: stream 40 → **Stream_Num_Bitfield[1]** bit 8
  - To apply the change to **all streams**, set **bit 0 of Stream_Num_Bitfield[0]**.
2. Set **Stream_Tx_Enable_State** to **1** (Enable) or **0** (Disable).
3. Apply the change by setting **Streams_Tx_Cfg_Apply** to **CODED_TRUE**.

**Note:** After the request/change is applied successfully, **Streams_Tx_Cfg_Apply** automatically returns to **CODED_FALSE**.
...
|0.9|04-March-2026| sjwpbv| Maulik| DNP-6969|
