...
function(create_compiler_common_configuration_target)
    ...
    list(APPEND msvc_cxx_flags "/EHsc") # Enable C++ exception handling, but only catch C++ exceptions (no SEH exceptions)
    ...
    target_compile_definitions(
        sg_compiler_config 
            INTERFACE 
                $<$<BOOL:${SG_DC_ENABLED}>: SG_DC_ENABLED>
                $<$<BOOL:${SG_DRIVABILITY_CLASSIFICATION_TOOLING_WRAPPER}>: SG_DC_SAVE_DETECTIONS_ASSIGNED_TO_SUBSEGMENTS>
    )

    ##############################################################################
    # Coverage
    ##############################################################################
    if(SG_DEVELOPER_MODE AND SG_UT)
        option(SG_CODE_COVERAGE "Code coverage" OFF)
        mark_as_advanced(SG_CODE_COVERAGE)
        if(SG_CODE_COVERAGE)
            include(cmake/SetupGcovCoverage.cmake)
            setup_gcov_coverage()
            target_link_libraries(
                sg_compiler_config
                    INTERFACE sg_gcov_coverage_setup
            )
        endif()
    endif()
endfunction()