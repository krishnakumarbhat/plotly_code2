THIS README NEEDS UPDATED TO REFLECT THE ROT CODE. RIGHT NOW IT IS JUST A COPY FROM A BUILDING BLOCK THAT PRECEEDS ROT



# Core Radar Calibration handler

[![pre-commit](https://img.shields.io/badge/pre--commit-enabled-brightgreen?logo=pre-commit&logoColor=white)](https://github.com/pre-commit/pre-commit)

This repository stores the calibration handler files, WORKSPACE,unit test files and BUILD files that are used with core radar projects. It also contains an example configuration which can be used by other projects to model BUILD file configuration, Workspace configuration, and verify the calibration handler builds.
...
Any updates to the configuration of this MCAL and/or BUILD files to build the MCAL shall be done in this repository.
...
This  repository contains an example configuration with a Build Env that can independently build the calibration handler code. The dependant files need to be fetched from jfrog artifactory. The example should be kept up to date with as much configuration as is practical to confirm nothing breaks in this repository.

However, an integration repository may be used in conjunction with this repository.
Local development can be done using the --override_repository flag in the integration repository.


    --override_repository={my local path}

can be included on the command line when building or a user.bazelrc file to compile this package

### Releasing New calibration handler for Integration

Releasing new calibration handler is completely automated through Jenkins, with the exception of updating the version.yaml file in this repo.
below listed workspaces need to be created in integration repo where calibration handler building block is integrated.
Please update local repository path as per the folder structure of integration repo.
The calibration specific dependencies (smc, usc, psc) may vary from project to project.
...
This scaled-down version contains only the necessary files to build the C implementation of the calibration handler. This includes all .c and .h files in the software\calib_handler\handler_files directory. It also includes the version.yaml file for tracking purposes. Uploading only these files reduces the size that Bazel needs to download and decreasing build times where the package needs to be downloaded.

Once the upload to Artifactory is complete, the commiter will get an email with details of how to use this new upload in a Bazel WORKSPACE file.

### Integrating calibration handler

### Build Commands
bazelisk build @example//:calb
