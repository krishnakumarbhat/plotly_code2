...
for i in range(count):
    ...
    if not CcaDevice_connect(dev, False):
        print('Failed to connect to device with IP: ' + str(ip_string))
        continue

    name_string = bytearray(30)
    if not CcaDevice_getName(dev, name_string, len(name_string)):
        print('Failed to get the name of the device with IP: ' + str(ip_string))
        continue

    print('{}: {}'.format( str(ip_string), str(name_string) ))
