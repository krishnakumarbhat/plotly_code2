...
# Map build customer names to folder names under Data/Aptiv_Libraries
CUSTOMER_DATA_DIR_MAP = {
    "CEER": "CEER",
    "GPO_Gen8": "GPO",
    "AL": "AL",
}
data_customer = CUSTOMER_DATA_DIR_MAP.get(customer, customer)
...
if os.path.exists(buildfmu_dir):
    if platform.system() == 'Windows':
        shutil.rmtree(buildfmu_dir, ignore_errors=True)
    else:
        # Linux/Mac: use rm -rf which is more reliable for NFS
        subprocess.run(f'rm -rf "{buildfmu_dir}"', shell=True, check=False)
...
fw_lime_path = os.path.join(project_root, 'Data', 'Aptiv_Libraries', 'FW_LIME', data_customer)
radar_dlls_path = os.path.join(project_root, 'Data', 'Aptiv_Libraries', 'RADAR_DLLS', data_customer)
dc_path = os.path.join(project_root, 'Data', 'Aptiv_Libraries', 'DC', data_customer)
