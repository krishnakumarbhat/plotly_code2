...
info: just enter a unused filename, do not use an existing file you still need since it will be overwritten. 
If you would like to use your own file for a test you can look at cca-IndexingOldFile example.
...
cca-device
----------
This program demonstrates how to scan the network for available cca devices.
...
Example: cca-set_current_time 
----------
Example file for the CCA-Framework: Demonstrates how to modify the current Logger time.
...
------------
Example application that uses CCA-Framework to upload a configuration (.cca file )to the Logger.
...
cca-writer_start_stop
------------
Example application that uses CCA-Framework to start and stop writer using CcaDevice_sendEvent and check their state.

Usage:
cca-writer_start_stop.exe [ip_address]
e.g.:
cca-writer_start_stop.exe 192.168.0.188
